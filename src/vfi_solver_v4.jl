#!/usr/bin/env julia
# vfi_solver_v4.jl — 6D state with proper tau_buy on every delta (Path B Option 1)
#
# State:    (t, w, z, ell, x_A_prev, x_B_prev)   — 6D
# Controls: (c, b, s, x_A_new, x_B_new)
#
# Key change vs v3: tau_buy charged on every positive delta, enabling genuine
# pre-buy hedge motive.  Pre-holding x_B while at A saves tau_buy at relocation.
#
...
#
# State update (x_prev for next period):
#   stay:             x_A_prev' = x_A_new,  x_B_prev' = x_B_new
#   relocate E2_2L:   same (tokens portable)
#   relocate E1_2L:   x_A_prev' = 0,         x_B_prev' = 0 (forced sale; buy next period)
#
# x_prev discretized at N_X_PREV points (default 3: {0.0, 0.5, 1.0}).
# x_new -> ix_prev_next via nearest-neighbor projection.
# Compute:  ~4.6x v3 per-regime wall time; run baselines on server1.
#
# Grid defaults (Option 1): N_W=15, N_Z=5, N_X_PREV=3, ASSET_GRID_SIZE=7, X_GRID_SIZE=5.
# Roughly 2.5 h per regime on server1 single thread.
...
struct GridSpec_v4
    ...
    n_x_prev::Int       # x_prev grid points (default 3)
...
struct Grids_v4
    w::Vector{Float64}
    z::Vector{Float64}
    x_prev::Vector{Float64}  # x_prev discretisation grid
end
...
function build_shock_block_v4(p::ModelParams_v4, cfg::SolveConfig_v4)
                    ...
                    for (i5,nh) in enumerate(nodes)
                        ...
                        for (i6,nu) in enumerate(nodes)
                            ...
                            for (i7,ne) in enumerate(nodes)
                                ...
                                hp[idx]  = hp_val;  u_v[idx] = u_val;   ep[idx]  = ep_val
    ...
    @assert idx == total
    ShockBlock_v4(rs, ra, rb, hp, u_v, ep, wts)
end
...
@inline function housing_cost_v4(x_A::Float64, x_B::Float64, ell::Int,
    ...
    else  # E2_2L: only occupied-unit token saves rent
        x_ell = ell == LOC_A ? x_A : x_B
...
@inline function interp_bilinear_v4(vals::AbstractMatrix{Float64},
    ...
    (1.0-f_w)*(1.0-f_z)*v11 + f_w*(1.0-f_z)*v21 +
    (1.0-f_w)*f_z*v12       + f_w*f_z*v22
end

# Nearest-neighbor projection of a continuous x value onto the x_prev grid.
@inline function nearest_xprev_idx(x::Float64, xpg::Vector{Float64})::Int
    n = length(xpg)
    n == 1 && return 1
    x <= xpg[1]   && return 1
    x >= xpg[end] && return n
    best = 1; best_d = abs(x - xpg[1])
...
# Compute transaction cost given previous and new x holdings.
...
# ix_A_stay/ix_B_stay: x_prev indices for next period if NO relocation
# ix_A_reloc/ix_B_reloc: x_prev indices for next period if relocation occurs
# ─────────────────────────────────────────────────────────────────────────────
...
function continuation_value_v4(
    ...
    @inbounds for q in eachindex(shock.weights)
        ...
        # Look up V at next (w, z) by bilinear interp; exact in (ell, ixA, ixB)
...
function solve_state_v4(
    ...
    nx      = cfg.x_grid_size
    xpg     = grids.x_prev  # x_prev grid

    # For E1_2L: on relocation, x_prev_next = (0, 0)
    ix_zero = 1  # index of 0.0 in x_prev_grid (grid starts at 0.0)

    if regime == REGIME_E0
        # No housing asset; x_prev_next = (1, 1) always.
        resources = w - p.rho
        resources <= 0.0 && return best_v, best_c, best_b, best_s, 0.0, 0.0, false
        # tx_cost for E0: x_A_new = x_B_new = 0, so sell any prev holdings
        tc = tx_cost_v4(p, x_A_prev, x_B_prev, 0.0, 0.0)
        resources -= tc
        resources <= 0.0 && return best_v, best_c, best_b, best_s, 0.0, 0.0, false
        # x_new = 0 → ix_next = 1 (0.0 in grid)
    ...
    elseif regime == REGIME_E1_2L
        # ── Rent (x_ell = 0) ─────────────────────────────────────────────────
        # Selling any prev holdings: x_A_new=0, x_B_new=0
        tc_rent  = tx_cost_v4(p, x_A_prev, x_B_prev, 0.0, 0.0)
        res_rent = w - p.rho - tc_rent
        if res_rent > 0.0
            ix_An = ix_zero;  ix_Bn = ix_zero  # next period: no holdings
            for b in candidate_grid_v4(res_rent, na)
                ...
                for s in candidate_grid_v4(max_s, na)
                    ...
                    ev = continuation_value_v4(p, grids, shock, f_profile,
                                               next_value_slice, t, z, ell,
                                               b, s, 0.0, 0.0,
                                               ix_An, ix_Bn,   # stay: no holdings
                                               ix_zero, ix_zero,  # reloc: forced sale → 0
                                               regime)
                    v = utility_crra(c, p.gamma) + p.beta * ev
                    if v > best_v
                        best_v, best_c, best_b, best_s = v, c, b, s
        ...
        # ── Own (x_ell = 1) ──────────────────────────────────────────────────
        # x_A_new / x_B_new at current location = 1; other location = 0
        x_A_own = ell == LOC_A ? 1.0 : 0.0
        x_B_own = ell == LOC_B ? 1.0 : 0.0
        tc_own  = tx_cost_v4(p, x_A_prev, x_B_prev, x_A_own, x_B_own)
        # Budget: c + m + 1 + tc + b + s = w
        ...
        if own_base > 0.0
            ...
            ix_An_own = nearest_xprev_idx(x_A_own, xpg)
            ix_Bn_own = nearest_xprev_idx(x_B_own, xpg)
            for b in b_cands
                b < b_lo && continue
                ...
                for s in candidate_grid_v4(max_s, na)
                    ...
                    ev = continuation_value_v4(p, grids, shock, f_profile,
                                               next_value_slice, t, z, ell,
                                               b, s, x_A_own, x_B_own,
                                               ix_An_own, ix_Bn_own,  # stay
                                               ix_zero,    ix_zero,   # reloc: forced sale
                                               regime)
                    v = utility_crra(c, p.gamma) + p.beta * ev
                    if v > best_v
                        best_v, best_c, best_b, best_s = v, c, b, s
        ...
        # Continuous (x_A, x_B) ≥ 0.  Parameterise as (X_total, alpha).
        # Budget: c + kappa(x_ell_new) + b + s + X_total + tx_cost = w
        delta_own = p.rho - p.m
        # Upper bound on X_total (ignoring tx_cost; tx_cost only tightens further)
        ...
        for X_total in X_grid
            for alpha in alpha_grid
                ...
                # x_prev indices for next period (tokens portable: same on stay and reloc)
                ix_An = nearest_xprev_idx(x_A_new, xpg)
                ix_Bn = nearest_xprev_idx(x_B_new, xpg)
                ...
                for b in b_cands
                    ...
                    for s in candidate_grid_v4(max_s, na)
                        ...
                        ev = continuation_value_v4(p, grids, shock, f_profile,
                                                   next_value_slice, t, z, ell,
                                                   b, s, x_A_new, x_B_new,
                                                   ix_An, ix_Bn,  # stay: tokens retained
                                                   ix_An, ix_Bn,  # reloc: tokens portable
                                                   regime)
                        v = utility_crra(c, p.gamma) + p.beta * ev
                        if v > best_v
                            best_v, best_c, best_b, best_s = v, c, b, s
...
# ─────────────────────────────────────────────────────────────────────────────
# Summary — reports at (x_A_prev=0, x_B_prev=0) initial state (ix=1,1)
# ─────────────────────────────────────────────────────────────────────────────
...
function summary_v4(result::SolverResult_v4, grids::Grids_v4,
    ...
    # Report at ix_A_prev=1, ix_B_prev=1 (entry state: no prior holdings)
    ix0 = 1
...
# ─────────────────────────────────────────────────────────────────────────────
# Smoke test — struct-init and mechanic checks only; no VFI run.
...
function smoke_test_v4()
    ...
    @printf("  tau_buy             = %.4f  (charged on every positive delta)\n", params.tau_buy)
    @printf("  tau_token           = %.4f  (charged on every negative delta)\n", params.tau_token)
    @printf("  tau_sell            = %.4f  (E1_2L relocation forced sale)\n", params.tau_sell)
    @printf("  sigma_div           = %.4f\n", params.sigma_div)
    @printf("  sigma_iota          = %.4f\n", params.sigma_iota)
    check_sigma = abs(sqrt(params.sigma_div^2 + params.sigma_iota^2) - params.sigma_h) < 1e-8
    println("  sigma decomposition OK: $check_sigma");  @assert check_sigma

    spec  = default_grids_v4(small=true)
    cfg   = default_config_v4(small=true)
    @printf("  grid: N_W=%d, N_Z=%d, N_X_PREV=%d, x_prev_max=%.1f\n",
            spec.n_w, spec.n_z, spec.n_x_prev, spec.x_prev_max)
    @printf("  choice: x_grid=%d, asset_grid=%d, GH_nodes=%d\n",
    ...
    nw   = spec.n_w; nz = spec.n_z; nx = spec.n_x_prev
    dims = (T, nw, nz, 2, nx, nx)
    n_states = prod(dims)
    mem_MB   = n_states * 8 * 7 / 1024^2   # 7 arrays × Float64
    @printf("  6D state array: %s  (%d states, %.1f MB for 7 arrays)\n",
            string(dims), n_states, mem_MB)
    @assert mem_MB < 500.0 "Memory exceeds 500 MB; reduce grid sizes"
    ...
    # tx_cost correctness
    p = params
    @assert tx_cost_v4(p, 0.0,  0.0,  1.0,  0.5) ≈ p.tau_buy * 1.5             "buy both"
    @assert tx_cost_v4(p, 1.0,  0.5,  0.5,  0.5) ≈ p.tau_token * 0.5            "sell A only"
    @assert tx_cost_v4(p, 0.5,  0.5,  0.5,  0.5) ≈ 0.0                          "no change"
    @assert tx_cost_v4(p, 1.0,  1.0,  0.0,  0.0) ≈ p.tau_token * 2.0            "sell all"
    @assert tx_cost_v4(p, 0.25, 0.0,  0.75, 0.5) ≈ p.tau_buy * (0.5 + 0.5)      "buy both"
    println("  tx_cost_v4 spot-checks: PASS")

    # nearest_xprev_idx correctness
    xpg = grids.x_prev  # e.g. [0.0, 0.5, 1.0]
    @assert nearest_xprev_idx(0.0,   xpg) == 1
    @assert nearest_xprev_idx(0.26,  xpg) == 2  # |0.26-0.5|=0.24 < |0.26-0.0|=0.26
    @assert nearest_xprev_idx(0.74,  xpg) == 2
    @assert nearest_xprev_idx(0.75,  xpg) == 3  # tie → rounds to last (3)
    @assert nearest_xprev_idx(1.0,   xpg) == 3
    @assert nearest_xprev_idx(99.0,  xpg) == 3  # capped at last
    println("  nearest_xprev_idx spot-checks: PASS")
    ...
    # housing_cost_v4 spot-checks (v4 uses same rule as fixed v3)
    ...
    @assert housing_cost_v4(0.0, 1.0, LOC_A, p, REGIME_E1_2L) == p.rho   # ell=A, x_B=1 → rent at A
    k_e2 = housing_cost_v4(0.9, 0.3, LOC_A, p, REGIME_E2_2L)
    # Only x_ell=x_A=0.9 saves rent at A; x_B purely financial.
    @assert abs(k_e2 - (p.rho - 0.9*(p.rho-p.m))) < 1e-12
    println("  housing_cost_v4 spot-checks: PASS")

    # x_prev state update logic (E1_2L relocation → zero)
    # After relocation in E1_2L, next period's x_prev_next = (0, 0) → ix = (1, 1)
    @assert nearest_xprev_idx(0.0, xpg) == 1  # ix_zero == 1
    println("  E1_2L relocation x_prev reset: ix=1 (0.0) confirmed")
    ...
    # Hedge premium estimate
    hedge_premium_per_period = p.p_relocate_working * p.tau_buy
    @printf("  expected hedge premium per unit x_B per period: %.4f (= p_reloc * tau_buy)\n",
            hedge_premium_per_period)
    @printf("  40-year working life cumulative (approx): %.3f\n",
            hedge_premium_per_period * 40)
...
function main_v4(args::Vector{String}=ARGS)
    ...
    @printf("  choice    : x_grid=%d, asset_grid=%d\n", cfg.x_grid_size, cfg.asset_grid_size)
    @printf("  quadrature: %d nodes, %d points total\n",
            cfg.quadrature_nodes, cfg.quadrature_nodes^7)
    @printf("  mobility  : p_reloc_work=%.3f, p_reloc_ret=%.3f\n",
            params.p_relocate_working, params.p_relocate_retired)
    @printf("  tx costs  : tau_sell=%.3f, tau_buy=%.3f (every delta), tau_token=%.3f\n",
