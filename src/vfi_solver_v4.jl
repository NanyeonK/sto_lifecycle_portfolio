#!/usr/bin/env julia
# vfi_solver_v4.jl — Path B Option 1: full state extension with x_prev tracking
# State: (t, w, z, ell, ix_A_prev, ix_B_prev)   — 6D
#
# Extends v3 by tracking prior-period token holdings (x_A_prev, x_B_prev) as
# discrete state dimensions. tau_buy is charged on positive deltas
#   tx_cost = tau_buy  * (max(x_A_new - x_A_prev, 0) + max(x_B_new - x_B_prev, 0))
#           + tau_token * (max(x_A_prev - x_A_new, 0) + max(x_B_prev - x_B_new, 0))
# This gives a genuine pre-holding hedge: household at ell=A can pre-buy x_B at
# cost tau_buy now, saving tau_buy * x_B on any future relocation to B.
#
# Regime taxonomy (same as v3):
#   E1_2L — binary own at current location; x_{ell'} = 0 always; sell on relocation
#   E2_2L — continuous fractional tokens of A and/or B; always portable
#
# x_prev grid: N_X_PREV points linearly spaced in [0, X_PREV_MAX].
# Choices for x_A_new and x_B_new are constrained to lie on the x_prev grid
# (simplifies state-transition to an exact index lookup with no interpolation).
#
# Default: N_X_PREV=3, X_PREV_MAX=2.0  →  x_prev_grid = {0.0, 1.0, 2.0}
# This covers the v3 empirical range (mean_x ≈ 1.748).
#
# Run:
#   REGIME=E1_2L julia src/vfi_solver_v4.jl       — binary-ownership baseline
#   REGIME=E2_2L julia src/vfi_solver_v4.jl       — full Option 1 with pre-holding
#   julia src/vfi_solver_v4.jl --smoke-test        — struct/shock checks (no VFI)
#
# Extra env vars beyond v3:
#   N_X_PREV   (int,   default 3)    — x_prev grid points per dimension
#   X_PREV_MAX (float, default 2.0)  — max x_prev value; must exceed typical x holdings
...
function regime_from_env_v4()
    ...
    if name == "E1_2L"
        return REGIME_E1_2L_V4
    elseif name == "E2_2L"
        return REGIME_E2_2L_V4
    else
...
struct Grids_v4
    w::Vector{Float64}
    z::Vector{Float64}
    x_prev::Vector{Float64}   # x_prev grid (shared for x_A_prev and x_B_prev)
end
...
function default_grids_v4(; small::Bool=true)
    if small
        return GridSpec_v4(
            parse(Int,     get(ENV, "N_W",   "15")),   # reduced per spec (was 21)
            parse(Float64, get(ENV, "W_MIN", "0.02")),
            parse(Float64, get(ENV, "W_MAX", "12.0")),
            parse(Int,     get(ENV, "N_Z",   "5")),    # reduced per spec (was 7)
...
# Corrected housing cost (occupied-unit only saves rent):
#   E1_2L: kappa = rho if x_ell < 1; m if x_ell >= 1
#   E2_2L: kappa = rho - x_ell_local * (rho - m)   [only occupied unit reduces rent]
...
# E1_2L: only tau_buy on positive delta of occupied-unit token;
#         tau_sell on forced relocation is handled separately via sell_factor.
# E2_2L: full spec — tau_buy on increases, tau_token on decreases, both locations.
@inline function tx_cost_v4(x_A_new::Float64, x_B_new::Float64,
                             x_A_prev::Float64, x_B_prev::Float64,
                             p::ModelParams_v4, regime::Int, ell::Int)::Float64
    if regime == REGIME_E1_2L_V4
        x_ell_new  = ell == LOC_A ? x_A_new  : x_B_new
        x_ell_prev = ell == LOC_A ? x_A_prev : x_B_prev
        return p.tau_buy * max(x_ell_new - x_ell_prev, 0.0)
    else   # E2_2L — full spec
...
# Wealth at t+1.  sell_factor_{A,B} = (1 - tau_sell) on forced relocation sell for E1_2L.
...
# ─────────────────────────────────────────────────────────────────────────────
# Continuation value — integrates shocks and relocation, looks up 6D next value
# ─────────────────────────────────────────────────────────────────────────────
#
# next_value_slice: view of result.value[t+1, :, :, :, :, :] — (N_W, N_Z, 2, N_xp, N_xp)
#
# x_A_new, x_B_new: current-period choices (lie on x_prev grid).
# ix_A_new, ix_B_new: their indices in grids.x_prev (1-based).
#   These become the x_A_prev, x_B_prev for the NEXT period.
#
# For E1_2L on relocation: sold x_ell is removed from next-period x_prev state.
#   Specifically, after relocating from A→B: x_A is forced-sold, so x_A_prev at t+1
#   should be 0 (index 1). x_B_prev at t+1 remains 0 (x_B was not held at A).
#   The sell proceeds are captured by sell_factor_A = (1 - tau_sell) in next_wealth.
#
function continuation_value_v4(
    ...
    # sell factors for relocation — only E1_2L has forced sale
    ...
    # x_prev indices for next period, stay vs. relocation branches
    ix_A_stay  = ix_A_new;  ix_B_stay  = ix_B_new     # no change on no-relocation
    ix_A_reloc = ix_A_new;  ix_B_reloc = ix_B_new     # default (E2_2L: tokens portable)
    if regime == REGIME_E1_2L_V4
        # After forced sell on relocation, the liquidated position becomes 0 in next-period state.
        # Tau_buy at NEW location is charged as a delta cost the NEXT period when the
        # household chooses x_ell_new > 0 at the new ell (x_ell_prev = 0 post-relocation).
        if ell == LOC_A
            ix_A_reloc = 1    # x_A_prev = 0 (sold A, moving to B)
        else
            ix_B_reloc = 1    # x_B_prev = 0 (sold B, moving to A)
        end
    end
...
function solve_state_v4(
    ...
    if regime == REGIME_E1_2L_V4
        # Binary at current location: x_ell_new ∈ {0, 1}; x_{ell'}_new = 0.
        # x=1 must be representable in x_prev grid; with default {0, 1, 2}: index 2.
        x_one_idx = searchsortedfirst(grids.x_prev, 1.0 - 1e-9)
        if x_one_idx > n_xp || abs(grids.x_prev[x_one_idx] - 1.0) > 1e-6
            x_one_idx = clamp(argmin(abs.(grids.x_prev .- 1.0)), 1, n_xp)
        end

        for (case, x_ell_new) in [(1, 0.0), (2, 1.0)]
            x_A_new = ell == LOC_A ? x_ell_new : 0.0
            x_B_new = ell == LOC_B ? x_ell_new : 0.0
            ix_A_new = ell == LOC_A ? (case == 1 ? 1 : x_one_idx) : 1
            ix_B_new = ell == LOC_B ? (case == 1 ? 1 : x_one_idx) : 1

            tc = tx_cost_v4(x_A_new, x_B_new, x_A_prev_val, x_B_prev_val, p, regime, ell)
        ...
        # Continuous fractional tokens; choices constrained to x_prev grid.
        # Enumerate all N_xp × N_xp = 9 (x_A_new, x_B_new) combinations.
        delta_own = p.rho - p.m
...
function terminal_slice_v4!(result::SolverResult_v4, p::ModelParams_v4,
    ...
    for (iw, w) in enumerate(grids.w),
        ...
        ixA in 1:n_xp,
        ixB in 1:n_xp
        # At terminal, x_prev doesn't affect terminal utility; household consumes w.
...
# ─────────────────────────────────────────────────────────────────────────────
# Summary — reports at initial state (t=1) with x_prev = 0 (fresh entrant)
# ─────────────────────────────────────────────────────────────────────────────
...
function summary_v4(result::SolverResult_v4, grids::Grids_v4,
    ...
    # Representative-state comparison: t=1, midpoint (w,z), ell=A, x_prev=(0,0)
    ...
    for (lbl, iell) in [("ellA", LOC_A), ("ellB", LOC_B)]
        # Summarise at t=1, x_prev=(0,0) (fresh-entrant slice — no prior holdings)
...
# ─────────────────────────────────────────────────────────────────────────────
# Smoke test — struct init, shock block, tx_cost, state allocation.
# Does NOT run VFI (cloud env may lack Julia; server1 runs VFI).
...
function smoke_test_v4()
    ...
    @printf("  decomp: sqrt(%.6f^2 + %.6f^2) = %.6f  (sigma_h = %.6f)\n",
    ...
    @assert length(grids.x_prev) == params.n_x_prev "x_prev grid length"
    @assert grids.x_prev[1] ≈ 0.0                   "x_prev[1] must be 0"
    @assert grids.x_prev[end] ≈ params.x_prev_max   "x_prev[end] must be x_prev_max"
    println("  x_prev grid checks: PASS")

    # 6D state array sizing
    result = initialize_result_v4(params, grids)
    T      = num_periods_v4(params) + 1
    dims   = size(result.value)
    @printf("  6D value array: %s  (T=%d, N_W=%d, N_Z=%d, N_ell=2, N_xp=%d, N_xp=%d)\n",
            string(dims), T, spec.n_w, spec.n_z, params.n_x_prev, params.n_x_prev)
    @assert ndims(result.value) == 6              "value must be 6D"
    @assert size(result.value, 1) == T            "T dimension"
    @assert size(result.value, 4) == 2            "ell dimension must be 2"
    @assert size(result.value, 5) == params.n_x_prev "ix_A_prev dimension"
    @assert size(result.value, 6) == params.n_x_prev "ix_B_prev dimension"
    mem_mb = prod(dims) * 8 * 6 / 1024^2   # 6 Float64 arrays
    @printf("  memory estimate (6 arrays): %.1f MB\n", mem_mb)
    println("  6D array size/memory: PASS")

    # Terminal slice
    terminal_slice_v4!(result, params, grids, T)
    @assert !any(isnan, result.value[T, :, :, :, :, :]) "NaN in terminal slice"
    @assert all(result.feasible[T, :, :, :, :, :])      "all terminal states feasible"
    ...
    @assert any(shock.ra .!= shock.rb)                    "R_A and R_B must differ"
    println("  shock block: PASS")

    # tx_cost spot-checks
    p = params
    # E2_2L: buying x_A from 0 to 1 costs tau_buy
    tc1 = tx_cost_v4(1.0, 0.0, 0.0, 0.0, p, REGIME_E2_2L_V4, LOC_A)
    @assert abs(tc1 - p.tau_buy) < 1e-12 "E2_2L buy x_A from 0→1: tx=$(tc1), expected $(p.tau_buy)"
    # E2_2L: buying x_B from 0 to 1 while at A costs tau_buy (pre-buying hedge)
    tc2 = tx_cost_v4(0.0, 1.0, 0.0, 0.0, p, REGIME_E2_2L_V4, LOC_A)
    @assert abs(tc2 - p.tau_buy) < 1e-12 "E2_2L buy x_B from 0→1 (hedge): tx=$(tc2)"
    # E2_2L: maintaining same holding → no cost
    tc3 = tx_cost_v4(1.0, 0.5, 1.0, 0.5, p, REGIME_E2_2L_V4, LOC_A)
    @assert abs(tc3) < 1e-12 "E2_2L no-change: tx=$(tc3), expected 0"
    # E2_2L: selling x_A from 1 to 0 costs tau_token
    tc4 = tx_cost_v4(0.0, 0.0, 1.0, 0.0, p, REGIME_E2_2L_V4, LOC_A)
    @assert abs(tc4 - p.tau_token) < 1e-12 "E2_2L sell x_A 1→0: tx=$(tc4), expected $(p.tau_token)"
    # E1_2L: buying x_A from 0 to 1 at ell=A costs tau_buy
    tc5 = tx_cost_v4(1.0, 0.0, 0.0, 0.0, p, REGIME_E1_2L_V4, LOC_A)
    @assert abs(tc5 - p.tau_buy) < 1e-12 "E1_2L buy x_A 0→1: tx=$(tc5)"
    # E1_2L: buying x_B (NOT at ell=A) should NOT generate buy cost (x_{ell'} is forced 0)
    tc6 = tx_cost_v4(0.0, 0.0, 1.0, 0.0, p, REGIME_E1_2L_V4, LOC_A)
    @assert abs(tc6) < 1e-12 "E1_2L sell x_A (reloc handled by sell_factor): tx=$(tc6)"
    println("  tx_cost spot-checks: PASS")

    # housing_cost spot-checks
    @assert housing_cost_v4(1.0, 0.0, LOC_A, p, REGIME_E1_2L_V4) == p.m    "E1_2L owner at A"
    @assert housing_cost_v4(0.0, 1.0, LOC_A, p, REGIME_E1_2L_V4) == p.rho  "E1_2L renter at A (x_B=1 ignored)"
    @assert housing_cost_v4(0.0, 0.0, LOC_A, p, REGIME_E2_2L_V4) ≈ p.rho   "E2_2L renter"
    expected_k = p.rho - 0.5 * (p.rho - p.m)
    @assert abs(housing_cost_v4(0.5, 0.0, LOC_A, p, REGIME_E2_2L_V4) - expected_k) < 1e-12 "E2_2L partial"
    println("  housing_cost spot-checks: PASS")

    # State transition integrity: x_A_prev for next period = x_A_new choice
    # Verify x_prev grid index for x=0 is 1, and x=1 is at some index ≤ n_x_prev
    @assert grids.x_prev[1] ≈ 0.0 "index 1 → x=0"
    x_one_idx = argmin(abs.(grids.x_prev .- 1.0))
    @assert abs(grids.x_prev[x_one_idx] - 1.0) < 1e-6 "x=1 representable in x_prev grid"
    println("  state transition integrity: PASS")
...
function main_v4(args::Vector{String}=ARGS)
    ...
    grids_tmp = build_grids_v4(grid_spec, params)
    T         = num_periods_v4(params) + 1
    n_xp      = params.n_x_prev
    total_states = (T * grid_spec.n_w * grid_spec.n_z * 2 * n_xp * n_xp)
    @printf("  grids      : N_W=%d, N_Z=%d, N_xp=%d (x_prev ∈ [0, %.1f])\n",
            grid_spec.n_w, grid_spec.n_z, n_xp, params.x_prev_max)
    @printf("  state count: T=%d × %d × %d × 2 × %d × %d = %d\n",
            T, grid_spec.n_w, grid_spec.n_z, n_xp, n_xp, total_states)
