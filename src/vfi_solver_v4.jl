...
# Option 1 full state extension — proper per-period transaction costs on x increments
#
# State:    (t, w, z, ell, ixA_prev, ixB_prev)
#   ell ∈ {LOC_A=1, LOC_B=2}
#   ixA_prev, ixB_prev ∈ 1..N_X_PREV (indices into coarse x_prev grid)
#
# Controls: regime-dependent:
#   E0      — (c, b, s)                  rent-only; no housing
...
#
# v4 changes vs v3:
#   1. Two new state dims: ixA_prev, ixB_prev — track previous-period token holdings
#   2. Per-period tx_cost in budget constraint:
...
#   3. 6D value function: (T, N_W, N_Z, N_ell, N_xA_prev, N_xB_prev)
#   4. Continuation value bilinear-interpolates over (w,z); nearest-grid on x_prev
#   5. E1_2L: sell_factor at relocation retained (tau_sell on housing return)
#   6. E2_2L: no sell_factor (tokens portable); only delta-based tx_cost
#
# Hedge mechanism: E2_2L household at ell=A pre-holds x_B_prev > 0.
#   On relocation to B they wake up with ixB_prev > 0 → delta_B = x_B_new - x_B_prev < 1
#   → saves tau_buy * x_B_prev vs buying from zero.
#
# Cloud env: DO NOT run heavy VFI (no Julia installed). Run --smoke-test only.
# Server1:   REGIME=E1_2L julia src/vfi_solver_v4.jl
...
struct ModelParams_v4
    ...
    tau_sell::Float64    # traditional selling cost ~6% NAR (E1_2L sell_factor)
    tau_buy::Float64     # buying cost ~2.5%; charged per positive delta in v4
    tau_token::Float64   # token transfer (decrease) cost ~0.5-1%
    # Mortgage
    ltv_max::Float64
...
struct GridSpec_v4
    ...
    z_min::Float64
    z_max::Float64
    n_xprev::Int        # points in x_prev grid (default 3: {0.0, 0.5, 1.0})
    xprev_max::Float64  # upper end of x_prev grid (default 1.5)
end
...
struct Grids_v4
    w::Vector{Float64}
    z::Vector{Float64}
    xprev::Vector{Float64}   # coarse x_prev grid
end
...
# Housing cost: same rules as v3 (fixed kappa — only occupied unit reduces rent)
@inline function housing_cost_v4(x_A::Float64, x_B::Float64, ell::Int,
    ...
    else  # E2_2L — only occupied-unit token reduces rent
        x_ell_local = ell == LOC_A ? x_A : x_B
        return p.rho - x_ell_local * (p.rho - p.m)
    end
end

# Transaction cost on x increments (v4 core mechanism)
...
# Wealth transition — no buy_ded_reloc (tx_cost now captured in period budget)
...
# ─────────────────────────────────────────────────────────────────────────────
# x_prev grid helper — nearest-neighbor projection
# ─────────────────────────────────────────────────────────────────────────────

@inline function nearest_idx_v4(xprev_grid::Vector{Float64}, x_val::Float64)::Int
    x_clamp = clamp(x_val, xprev_grid[1], xprev_grid[end])
    best_i  = 1
    best_d  = abs(xprev_grid[1] - x_clamp)
    @inbounds for i in 2:length(xprev_grid)
        d = abs(xprev_grid[i] - x_clamp)
        if d < best_d
            best_d = d
            best_i = i
        end
...
# x_A_new, x_B_new: CURRENT period's chosen holdings (become next period's x_prev)
...
function continuation_value_v4(
    ...
    # Next-period x_prev indices from current choice (nearest-neighbor)
    ixA_next = nearest_idx_v4(grids.xprev, x_A_new)
    ixB_next = nearest_idx_v4(grids.xprev, x_B_new)

    # Sell factors: E1_2L loses tau_sell on forced sale at relocation;
    # E2_2L tokens are portable — no sell_factor applied.
    ...
    @inbounds for q in eachindex(shock.weights)
        ...
        # Bilinear over (w, z) for fixed (ell, ixA_next, ixB_next)
...
# x_A_prev, x_B_prev: continuous values from x_prev grid (already indexed)
# ─────────────────────────────────────────────────────────────────────────────
...
function solve_state_v4(
    ...
    x_A_prev = grids.xprev[ixA_prev]
    x_B_prev = grids.xprev[ixB_prev]
    ...
    if regime == REGIME_E0
        # No housing asset; no tx_cost (x_A_new = x_B_new = 0 always)
        # tx_cost would be tau_token * (x_A_prev + x_B_prev) for selling prior holdings,
        # but E0 is a separate regime where household never held x. Skip.
        resources = w - p.rho
    ...
    elseif regime == REGIME_E1_2L
        ...
        # If ell=A: x_A_new=0, x_B_new=0
        #   dA = 0 - x_A_prev, dB = 0 - 0 (x_B_prev=0 in E1_2L by construction)
        #   tx_cost = tau_token * max(-dA, 0) = tau_token * x_A_prev
        x_A_new_rent = 0.0
        x_B_new_rent = 0.0
        ...
        # x_A_new=1 if ell=A, x_B_new=0; or x_A_new=0, x_B_new=1 if ell=B
        ...
        # Continuous (x_A_new, x_B_new) ≥ 0 with tx_cost on deltas.
        # Grid: X_total ∈ [0, max_X], alpha ∈ [0,1].
        # x_A_new = alpha * X_total, x_B_new = (1-alpha) * X_total.
        # kappa depends only on x_ell_local (occupied unit).
        # tx_cost depends on deltas from x_prev.
        delta_own = p.rho - p.m
        net_cost  = 1.0 - delta_own   # 1 - (rho - m); cost of X_total net of rent saving
        # Conservative upper bound on X_total (ignoring tx_cost; tightened in loop)
...
function solve_v4(;
    ...
    for t in (t_last - 1):-1:1
        ...
        # next_value_slice: (N_W, N_Z, 2, N_xprev, N_xprev)
    ...
    result.metadata["state_definition"]    = "(t, w, z, ell, ixA_prev, ixB_prev)"
    result.metadata["control_definition"]  = "(c, b, s, x_A_new, x_B_new)"
    result.metadata["n_xprev"]             = grid_spec.n_xprev
...
# ─────────────────────────────────────────────────────────────────────────────
# Summary — reports at initial x_prev = 0 slice (entry state)
# ─────────────────────────────────────────────────────────────────────────────

function summary_v4(result::SolverResult_v4, grids::Grids_v4,
                    params::ModelParams_v4, regime::Int, grid_spec::GridSpec_v4)
    s = Dict{String,Any}()
    ...
    s["n_xprev"]         = grid_spec.n_xprev
    s["xprev_grid"]      = collect(grids.xprev)

    iw_mid = max(1, div(length(grids.w), 2))
    iz_mid = max(1, div(length(grids.z), 2))
    # Entry state: ixA_prev=1, ixB_prev=1 (both at grid[1] = 0.0)
    ix0 = 1
...
function smoke_test_v4()
    ...
    println("  State: (t, w, z, ell, ixA_prev, ixB_prev)  6D")

    params = default_params_v4()
    @printf("  rho_AB             = %.2f\n",  params.rho_AB)
    @printf("  tau_sell           = %.4f\n",  params.tau_sell)
    @printf("  tau_buy            = %.4f  (per-period delta cost, v4 core)\n", params.tau_buy)
    @printf("  tau_token          = %.4f  (per-period delta cost, v4 core)\n", params.tau_token)
    @printf("  sigma_div          = %.4f\n",  params.sigma_div)
    ...
    @printf("  control: asset_grid=%d, x_grid=%d, GH_nodes=%d\n",
    ...
    @assert grids.xprev[1]      == 0.0  "x_prev grid must start at 0"
    println("  xprev grid: $(grids.xprev)")
    ...
    n_elem = prod(dims)
    mem_mb = n_elem * 8 / 1e6  # Float64 = 8 bytes
    @printf("  6D value array: %s = %d elements, %.2f MB\n", string(dims), n_elem, mem_mb)
    ...
    # tx_cost_v4 correctness checks
    #   1. Buying x_B from 0 to 0.5: cost = tau_buy * 0.5
    tc1 = tx_cost_v4(0.0, 0.5, 0.0, 0.0, params)
    @assert abs(tc1 - params.tau_buy * 0.5) < 1e-12  "tx_cost buy-only check failed"
    #   2. Selling x_A from 1.0 to 0: cost = tau_token * 1.0
    tc2 = tx_cost_v4(0.0, 0.0, 1.0, 0.0, params)
    @assert abs(tc2 - params.tau_token * 1.0) < 1e-12 "tx_cost sell-only check failed"
    #   3. No change: tx_cost = 0
    tc3 = tx_cost_v4(0.5, 0.3, 0.5, 0.3, params)
    @assert tc3 == 0.0 "tx_cost identity check failed"
    #   4. Simultaneous buy A, sell B
    tc4 = tx_cost_v4(0.8, 0.0, 0.5, 0.3, params)
    expected4 = params.tau_buy * 0.3 + params.tau_token * 0.3
    @assert abs(tc4 - expected4) < 1e-12 "tx_cost mixed check failed"
    println("  tx_cost_v4 checks: PASS")

    # nearest_idx_v4 checks
    xpg = grids.xprev  # e.g. [0.0, 0.75, 1.5] for N_X_PREV=3, xprev_max=1.5
    @assert nearest_idx_v4(xpg, 0.0)         == 1
    @assert nearest_idx_v4(xpg, xpg[end])    == spec.n_xprev
    @assert nearest_idx_v4(xpg, -1.0)        == 1       "clamping to grid min"
    @assert nearest_idx_v4(xpg, 999.0)       == spec.n_xprev  "clamping to grid max"
    println("  nearest_idx_v4 checks: PASS")
    ...
    @assert size(result.value, 5) == nxp          "ixA_prev dimension wrong"
    @assert size(result.value, 6) == nxp          "ixB_prev dimension wrong"

    # Terminal slice
    terminal_slice_v4!(result, params, grids, spec, T)
    @assert all(result.feasible[T, :, :, :, :, :]) "terminal states infeasible"
    @assert !any(isnan, result.value[T, :, :, :, :, :]) "NaN in terminal slice"
    println("  6D array + terminal slice: PASS")

    # State update consistency: x_A_prev = x_A_new after one period
    # Verify nearest_idx round-trips cleanly for grid points
    for (ix, xv) in enumerate(grids.xprev)
        idx = nearest_idx_v4(grids.xprev, xv)
        @assert idx == ix "nearest_idx not identity on grid points (ix=$ix, got $idx)"
    end
    println("  x_prev round-trip on grid points: PASS")

    # housing_cost_v4 spot-checks (same rules as v3)
    p = params
    @assert housing_cost_v4(0.0, 0.0, LOC_A, p, REGIME_E0)    == p.rho
    @assert housing_cost_v4(1.0, 0.0, LOC_A, p, REGIME_E1_2L) == p.m   # owner at A
    @assert housing_cost_v4(0.0, 1.0, LOC_A, p, REGIME_E1_2L) == p.rho # B-token ≠ ownership at A
    ...
    println()
    println("  Hedge mechanism design summary:")
    xprev_mid = length(grids.xprev) >= 2 ? grids.xprev[2] : grids.xprev[end]
    hedge_saving_per_reloc = params.tau_buy * xprev_mid
    expected_lifetime_gain = params.p_relocate_working * hedge_saving_per_reloc
    @printf("  Pre-holding xB=%.2f at ell=A saves tau_buy*xB=%.4f per relocation.\n",
            xprev_mid, hedge_saving_per_reloc)
    @printf("  At p_reloc=%.2f: expected hedge saving %.4f per period (%.2f%% of wealth).\n",
            params.p_relocate_working, expected_lifetime_gain, expected_lifetime_gain * 100.0)

    println()
    println("=== smoke_test_v4: ALL PASS ===")
    println()
    println("Next steps (run on server1):")
    println("  REGIME=E1_2L julia src/vfi_solver_v4.jl")
...
function main_v4(args::Vector{String}=ARGS)
    ...
    @printf("  6D state: %s  (%.1f MB per array × 7 arrays)\n", string(dims), mem_mb)
    @printf("  mobility : p_reloc_work=%.3f, p_reloc_ret=%.3f\n",
            params.p_relocate_working, params.p_relocate_retired)
    @printf("  tx costs : tau_sell=%.3f, tau_buy=%.3f, tau_token=%.4f (per-period delta)\n",
            params.tau_sell, params.tau_buy, params.tau_token)
    @printf("  returns  : rho_AB=%.2f, sigma_div=%.4f, sigma_iota=%.4f\n",
            params.rho_AB, params.sigma_div, params.sigma_iota)
    @printf("  x_prev   : N_X_PREV=%d, xprev_max=%.1f\n", nxp, grid_spec.xprev_max)
    flush(stdout)
