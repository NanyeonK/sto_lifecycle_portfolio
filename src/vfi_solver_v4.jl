...
#
# Key changes vs v3 (4D state):
#   1. x_A_prev, x_B_prev added as explicit state dimensions.
#      At period t, x_prev records holdings CARRIED INTO the period
#      (= x_new from period t-1).  At t=1, x_A_prev = x_B_prev = 0.
#   2. Transaction costs charged in BUDGET CONSTRAINT on per-period deltas:
#        delta_A = x_A_new - x_A_prev
#        delta_B = x_B_new - x_B_prev
#        tx_cost = tau_buy   * (max(delta_A,0) + max(delta_B,0))   [positive increment]
#               + sell_rate * (max(-delta_A,0) + max(-delta_B,0))  [reduction]
#      sell_rate = tau_sell  for E1_2L (NAR ~6%)
#      sell_rate = tau_token for E2_2L (blockchain transfer ~1%)
#   3. Wealth transition simplified: no sell_factor (tx costs are in budget).
#      Approximation: sell cost charged on unit count, not on realized value.
#      First-order approx; add note in paper.
#   4. x choices CONSTRAINED to x_prev_grid (N_X_PREV points in [0, X_PREV_MAX]).
#      Default: N_X_PREV=3, X_PREV_MAX=1.0 → {0, 0.5, 1.0}.
#      This enables exact discrete state tracking without x_prev interpolation.
#      Grid can be refined (increase N_X_PREV) for higher-fidelity runs.
#
# Hedge mechanism (restored):
#   Pre-holding x_B_new > 0 at ell=A avoids paying tau_buy when relocating to B
#   in future periods.  Expected premium per unit of x_B held at ell=A:
#   p_relocate × tau_buy ≈ 0.06 × 0.025 = 0.15% per period.
#
# Spec: handoff/tau_buy_option1_spec.md
# Research: research_log.md 2026-05-01 (mechanism dead) and 2026-05-02 (Option 1 design)
...
struct ModelParams_v4
    ...
    tau_sell::Float64    # E1_2L sell cost (~6%, NAR)
    tau_buy::Float64     # buy cost, both regimes (~2.5%)
    tau_token::Float64   # E2_2L token sell cost (~1%)
...
struct SolveConfig_v4
    asset_grid_size::Int   # b and s candidate points
    quadrature_nodes::Int  # GH nodes per dimension (3 or 5)
    small_grid_mode::Bool
    save_path::Union{Nothing,String}
    n_x_prev::Int          # x_prev grid points per location (default 3)
    x_prev_max::Float64    # max x per location (default 1.0; must be ≥ 1.0 for E1_2L)
end
...
struct ShockBlock_v4
    ...
    hp::Vector{Float64}    # house-price scale exp(g_h + xi)
...
function build_grids_v4(gs::GridSpec_v4, cfg::SolveConfig_v4)
    w      = collect(gs.w_min .+ (gs.w_max - gs.w_min) .* (range(0.0, 1.0; length=gs.n_w) .^ 3.0))
    z      = collect(exp.(range(log(gs.z_min), log(gs.z_max); length=gs.n_z)))
...
@inline function p_relocate_v4(p::ModelParams_v4, t::Int)::Float64
    return (p.age0 + t - 1) <= p.retire_age ? p.p_relocate_working : p.p_relocate_retired
end

# FIXED kappa rule: only the OCCUPIED-location token reduces rent.
# x_{non-occupied} is purely a financial/hedge asset; no rental income assumed.
# This is the economically correct rule (corrected from v3's initial double-counting bug).
...
# Transaction cost on delta x = x_new - x_prev.
# Sell side: tau_sell (E1_2L, NAR commission ~6%) vs tau_token (E2_2L, blockchain ~1%).
# Buy side: tau_buy (~2.5%) for both regimes.
@inline function compute_tx_cost(x_A_prev::Float64, x_B_prev::Float64,
    ...
    sell_cost = sell_rate * (max(-dA, 0.0) + max(-dB, 0.0))
...
function next_income_state_v4(p::ModelParams_v4, f_profile, t::Int,
                                z::Float64, hp_next::Float64,
...
# Wealth transition: no sell_factor adjustment (tx costs are in the budget constraint).
# Note: sell cost approximated at unit price (not realized value); see file header.
...
@inline function interp_bilinear_v4(vals::AbstractMatrix{Float64},
    ...
    return (1-f_w)*(1-f_z)*v11 + f_w*(1-f_z)*v21 + (1-f_w)*f_z*v12 + f_w*f_z*v22
end
...
# next_value_slice: view of result.value[t+1, :, :, :, :, :] — shape (n_w, n_z, n_ell, n_xA, n_xB)
# ix_xA_new, ix_xB_new: indices into x_prev_grid for the CHOSEN x_new (= x_prev at t+1).
# Wealth transition is the same for stay and reloc (no sell_factor);
# ell difference at t+1 is handled by choosing the right value slice.
function continuation_value_v4(
    p::ModelParams_v4, grids::Grids_v4, shock::ShockBlock_v4, f_profile,
    ...
    @inbounds for q in eachindex(shock.weights)
        ...
        v_stay  = interp_bilinear_v4(
            view(next_value_slice, :, :, ell,     ix_xA_new, ix_xB_new),
            grids.w, grids.z, w_next, z_next)
        v_reloc = interp_bilinear_v4(
            view(next_value_slice, :, :, ell_alt, ix_xA_new, ix_xB_new),
            grids.w, grids.z, w_next, z_next)
...
# Nearest-neighbor index of val in grid.
function find_ix(val::Float64, grid::Vector{Float64})::Int
    best = 1;  best_d = abs(grid[1] - val)
    for i in 2:length(grid)
        d = abs(grid[i] - val)
        if d < best_d;  best_d = d;  best = i;  end
    end
    return best
end
...
function solve_state_v4(
    ...
    if regime == REGIME_E0
        # No housing asset; x_new = (0, 0); tx_cost trivially 0 if x_prev also 0.
        # If arriving with x_prev > 0 (shouldn't occur in E0 steady-state, but handled):
        # we don't charge tx_cost since E0 doesn't hold x. Guard by using 0,0 as x_new.
        resources = w - p.rho
    ...
    elseif regime == REGIME_E1_2L
        # Binary ownership at current location only.
        # Candidate (x_A_new, x_B_new, ix_A, ix_B):
        #   rent anywhere:  (0, 0)
        #   own at A:       (1, 0)  only valid if ell=A
        #   own at B:       (0, 1)  only valid if ell=B
        ix0 = find_ix(0.0, xpg)
        ix1 = find_ix(1.0, xpg)

        e1_cands = [(0.0, 0.0, ix0, ix0)]
        if ell == LOC_A
            push!(e1_cands, (1.0, 0.0, ix1, ix0))
        else
            push!(e1_cands, (0.0, 1.0, ix0, ix1))
        end

        for (x_A_new, x_B_new, ix_A, ix_B) in e1_cands
            tx    = compute_tx_cost(x_A_prev, x_B_prev, x_A_new, x_B_new, regime, p)
        ...
        # Continuous fractional tokens at either or both locations.
        # x choices constrained to x_prev_grid × x_prev_grid for exact state tracking.
        # Higher fidelity: increase N_X_PREV and/or X_PREV_MAX.
        for (ix_A, x_A_new) in enumerate(xpg)
            for (ix_B, x_B_new) in enumerate(xpg)
                tx    = compute_tx_cost(x_A_prev, x_B_prev, x_A_new, x_B_new, regime, p)
                kappa = housing_cost_v4(x_A_new, x_B_new, ell, p, regime)
                res   = w - kappa - x_A_new - x_B_new - tx
                res <= 0.0 && continue
...
function initialize_result_v4(p::ModelParams_v4, grids::Grids_v4)
    ...
    n_w  = length(grids.w); n_z = length(grids.z); n_xp = length(grids.x_prev)
    dims = (T, n_w, n_z, 2, n_xp, n_xp)
    nbytes = prod(dims) * 8
    @printf("  6D state: T=%d × n_w=%d × n_z=%d × n_ell=2 × n_xA=%d × n_xB=%d\n",
            T, n_w, n_z, n_xp, n_xp)
    @printf("  Memory per Float64 6D array: %.1f MB (7 arrays ≈ %.1f MB)\n",
            nbytes/1024^2, 7*nbytes/1024^2)
...
function solve_v4(;
    ...
    for t in (t_last - 1):-1:1
        ...
        next_slice = view(result.value, t+1, :, :, :, :, :)  # 5D (n_w, n_z, n_ell, n_xA, n_xB)
        for (iw, w) in enumerate(grids.w),
    ...
    result.metadata["solver_version"]    = "v4"
...
function summary_v4(result::SolverResult_v4, grids::Grids_v4,
    ...
    s["solver_version"]   = "v4"
    s["state_dims"]       = "(t, w, z, ell, x_A_prev, x_B_prev)"
    ...
    ix0    = 1  # x_prev=(0,0): initial-condition slice

    # Midpoint value at x_prev=(0,0) — directly comparable to v3 midpoint
    ...
    for (lbl, iell) in [("ellA", LOC_A), ("ellB", LOC_B)]
        ...
        s["V_t1_mean_feas_$(lbl)_x0"]  = isempty(feas_v) ? nothing : mean(feas_v)
        s["mean_xA_t1_$(lbl)_x0"]      = isempty(feas_v) ? nothing : mean(xAp[f1])
        s["mean_xB_t1_$(lbl)_x0"]      = isempty(feas_v) ? nothing : mean(xBp[f1])
        s["xB_gt0_count_$(lbl)_x0"]    = count(x -> x > 0.0, xBp[f1])
        # xB > 0 at ell=A is the hedge signature (H1 hypothesis)
    end
...
# ─────────────────────────────────────────────────────────────────────────────
# Smoke test — struct-init and consistency checks; VFI not run.
...
function smoke_test_v4()
    ...
    @printf("  x_prev_grid  (%d pts, max=%.1f): %s\n",
            cfg.n_x_prev, cfg.x_prev_max, string(grids.x_prev))
    @printf("  grid dims    : n_w=%d, n_z=%d, n_ell=2, n_xA=%d, n_xB=%d\n",
            length(grids.w), length(grids.z), length(grids.x_prev), length(grids.x_prev))
    @printf("  state total  : T=%d × %d × %d × 2 × %d × %d = %d\n",
            num_periods_v4(params)+1,
            length(grids.w), length(grids.z),
            length(grids.x_prev), length(grids.x_prev),
            (num_periods_v4(params)+1)*length(grids.w)*length(grids.z)*2*
            length(grids.x_prev)*length(grids.x_prev))
    ...
    @assert size(result.value, 5) == cfg.n_x_prev "x_A_prev dim wrong"
    @assert size(result.value, 6) == cfg.n_x_prev "x_B_prev dim wrong"
    println("  [1] 6D array allocation: PASS")

    # ── 2. Terminal slice ─────────────────────────────────────────────────────
    T = num_periods_v4(params) + 1
    terminal_slice_v4!(result, params, grids, T)
    @assert !any(isnan, result.value[T, :, :, :, :, :])   "NaN in terminal slice"
    @assert all(result.feasible[T, :, :, :, :, :])         "some terminal infeasible"
    println("  [2] Terminal slice: PASS")

    # ── 3. tx_cost computation ────────────────────────────────────────────────
    # buy 1 unit of A (from 0): tau_buy
    @assert compute_tx_cost(0.0, 0.0, 1.0, 0.0, REGIME_E2_2L, params) ≈ params.tau_buy
    # sell 1 unit of A (E2_2L, from 1 to 0): tau_token
    @assert compute_tx_cost(1.0, 0.0, 0.0, 0.0, REGIME_E2_2L, params) ≈ params.tau_token
    # E1_2L relocation: sell A (tau_sell), buy B (tau_buy)
    @assert compute_tx_cost(1.0, 0.0, 0.0, 1.0, REGIME_E1_2L, params) ≈ params.tau_sell + params.tau_buy
    # no change: zero cost
    @assert compute_tx_cost(1.0, 0.0, 1.0, 0.0, REGIME_E1_2L, params) ≈ 0.0
    # partial buy of B (E2_2L, from 0 to 0.5): tau_buy * 0.5
    @assert compute_tx_cost(0.5, 0.0, 0.5, 0.5, REGIME_E2_2L, params) ≈ params.tau_buy * 0.5
    println("  [3] tx_cost computation (5 checks): PASS")

    # ── 4. x_prev = x_new identity → zero tx_cost ────────────────────────────
    for x in grids.x_prev
        @assert compute_tx_cost(x, x, x, x, REGIME_E2_2L, params) ≈ 0.0
        @assert compute_tx_cost(x, x, x, x, REGIME_E1_2L, params) ≈ 0.0
    end
    println("  [4] x_prev=x_new identity (zero tx): PASS")
    ...
    @printf("  [5] Shock block (%d points, weight_sum=%.8f): PASS\n",
            length(shock.weights), sum(shock.weights))

    # ── 6. Sigma decomposition ────────────────────────────────────────────────
    @assert abs(sqrt(params.sigma_div^2 + params.sigma_iota^2) - params.sigma_h) < 1e-8
    println("  [6] Sigma decomposition: PASS")

    # ── 7. Housing cost (FIXED kappa rule) ────────────────────────────────────
    @assert housing_cost_v4(0.0, 0.0, LOC_A, params, REGIME_E0)    == params.rho
    @assert housing_cost_v4(1.0, 0.0, LOC_A, params, REGIME_E1_2L) == params.m      # owner at A
    @assert housing_cost_v4(0.0, 1.0, LOC_A, params, REGIME_E1_2L) == params.rho    # x_B not at ell=A
    @assert housing_cost_v4(0.0, 1.0, LOC_B, params, REGIME_E1_2L) == params.m      # owner at B
    kappa_e2_A = housing_cost_v4(0.5, 0.5, LOC_A, params, REGIME_E2_2L)
    expected_A = params.rho - 0.5 * (params.rho - params.m)                           # only x_A saves rent at ell=A
    @assert abs(kappa_e2_A - expected_A) < 1e-12
    kappa_e2_B = housing_cost_v4(0.5, 0.5, LOC_B, params, REGIME_E2_2L)
    expected_B = params.rho - 0.5 * (params.rho - params.m)                           # only x_B saves rent at ell=B
    @assert abs(kappa_e2_B - expected_B) < 1e-12
    println("  [7] Housing cost (FIXED kappa, 6 checks): PASS")

    # ── 8. find_ix ────────────────────────────────────────────────────────────
    @assert find_ix(0.0, grids.x_prev) == 1
    @assert find_ix(cfg.x_prev_max, grids.x_prev) == cfg.n_x_prev
    println("  [8] find_ix: PASS")

    # ── 9. E1_2L requires 0 and 1 in x_prev_grid ─────────────────────────────
    ix0 = find_ix(0.0, grids.x_prev)
    ix1 = find_ix(1.0, grids.x_prev)
    @assert abs(grids.x_prev[ix0] - 0.0) < 1e-8 "0.0 not in x_prev_grid at ix0"
    @assert abs(grids.x_prev[ix1] - 1.0) < 1e-8 "1.0 not found in x_prev_grid (X_PREV_MAX must be ≥ 1.0)"
    println("  [9] E1_2L index lookup (0 and 1 in x_prev_grid): PASS")

    # ── 10. p_relocate boundary checks ───────────────────────────────────────
    ...
    println("  [10] p_relocate boundary: PASS")
...
function main_v4(args::Vector{String}=ARGS)
    ...
    grids_preview = build_grids_v4(grid_spec, cfg)
    @printf("  grids     : N_W=%d, N_Z=%d\n", grid_spec.n_w, grid_spec.n_z)
    @printf("  x_prev    : N_X_PREV=%d, X_PREV_MAX=%.1f, grid=%s\n",
            cfg.n_x_prev, cfg.x_prev_max, string(grids_preview.x_prev))
    @printf("  quadrature: %d nodes, %d^7=%d points\n",
            cfg.quadrature_nodes, cfg.quadrature_nodes, cfg.quadrature_nodes^7)
    @printf("  tx costs  : tau_sell=%.3f (E1_2L), tau_buy=%.3f, tau_token=%.3f (E2_2L)\n",
