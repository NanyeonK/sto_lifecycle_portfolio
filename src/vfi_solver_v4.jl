#!/usr/bin/env julia
# vfi_solver_v4.jl — 6D state extension: full per-period transaction costs
# State:    (t, w, z, ell, x_A_prev, x_B_prev)
# x choices restricted to coarse x_prev grid (N_X_PREV=3 default: {0.0, 0.5, 1.0})
# so continuation-value lookup is exact (no interpolation in x_prev dimensions).
#
# Transaction cost on position adjustments (applied at choice time, every period):
#   delta_A   = x_A_new - x_A_prev
#   delta_B   = x_B_new - x_B_prev
#   tx_cost   = tau_buy   * (max(delta_A,0) + max(delta_B,0))
#             + tau_token * (max(-delta_A,0) + max(-delta_B,0))
#
# Why this activates the hedge channel:
#   E2_2L household at ell=A can pre-hold x_B > 0, paying tau_buy incrementally.
#   On relocation to B, x_B is already held — no additional buying cost.
#   Expected saving per unit x_B held: p_relocate * tau_buy ≈ 0.06*0.025 = 0.0015/yr.
#
# E1_2L: x_ell ∈ {0, 1} (binary at current location); x_{ell'} = 0 always.
#   Forced relocation sale: sell_factor (1-tau_sell) applied in wealth transition.
#   Post-relocation purchase: next period at new location with x_prev=(0,0),
#     choosing x_ell_new = own_amt triggers tx_cost = tau_buy (naturally via delta).
#   Voluntary sell (x_prev=1 → x_new=0): costs tau_token (approx; seldom optimal).
#
# E2_2L: x_A, x_B each from x_prev grid; tokens portable across relocation.
#   No sell_factor; tx_cost applied on all deltas.
#
...
struct SolveConfig_v4
    ...
    n_x_prev::Int         # x_prev grid resolution (default 3: {0, 0.5, 1.0})
...
struct Grids_v4
    w::Vector{Float64}
    z::Vector{Float64}
    x_prev::Vector{Float64}   # coarse x_prev grid; choices are restricted to these points
end
...
function default_grids_v4(; small::Bool=true)
    if small
        # Reduced N_W=15, N_Z=5 to compensate for 6D state expansion (spec: tau_buy_option1_spec.md)
        return GridSpec_v4(
...
function build_grids_v4(s::GridSpec_v4, cfg::SolveConfig_v4)
    return Grids_v4(build_w_grid_v4(s), build_z_grid_v4(s), build_x_prev_grid_v4(cfg))
end
...
function build_shock_block_v4(p::ModelParams_v4, cfg::SolveConfig_v4)
    ...
    # Cholesky decomposition for bivariate (iota_A, iota_B) with correlation rho_AB
...
# Per-period transaction cost on position adjustments.
# Positive delta (buying more): tau_buy per unit.
# Negative delta (selling): tau_token per unit (for E2_2L token sells; E1_2L forced
#   relocation sales are handled separately via sell_factor in the wealth transition).
...
# Net housing cost — only the OCCUPIED location's token reduces rent (corrected rule).
# E1_2L: binary (x_ell > 0 means owning one unit, paying maintenance m).
# E2_2L: smooth — occupied token saves delta_own per unit held.
...
# ─────────────────────────────────────────────────────────────────────────────
# Continuation value — 6D state; exact lookup in x_prev dimensions
# ─────────────────────────────────────────────────────────────────────────────

# next_value_slice: view of result.value[t+1, :, :, :, :, :] — shape (n_w, n_z, 2, n_xp, n_xp)
# ix_A_new, ix_B_new: 1-based indices of chosen x_A_new, x_B_new in grids.x_prev.
#
# State transition for x_prev dimension:
#   E2_2L: tokens portable — ix_A/B at new location = ix_A/B_new (same positions carried)
#   E1_2L: forced sale at relocation — ix_A/B resets to 1 (= 0.0) at new location
#          (sell_factor in wealth accounts for the 6% cost; x_prev reflects post-sale position)
function continuation_value_v4(
    ...
    # Sell factors and x_prev indices for the relocation case
    sf_A_reloc = 1.0;  sf_B_reloc = 1.0
    ix_A_reloc = ix_A_new;  ix_B_reloc = ix_B_new   # E2_2L: tokens portable

    if regime == REGIME_E1_2L
        # Forced sale of the currently-occupied unit when relocating
        if ell == LOC_A
            sf_A_reloc = 1.0 - p.tau_sell
        else
            sf_B_reloc = 1.0 - p.tau_sell
        end
        # After forced sale, household arrives at new location with zero prior holdings.
        # Index 1 corresponds to x_prev_grid[1] = 0.0.
        ix_A_reloc = 1
        ix_B_reloc = 1
    end
...
function solve_state_v4(
    ...
    if regime == REGIME_E0
        # Pure renter; x choices are always 0 regardless of x_prev.
        resources = w - p.rho
    ...
    elseif regime == REGIME_E1_2L
        # Binary x_ell ∈ {0, own_amt}; x_{ell'} = 0 always.
        # "own_amt" = grid point nearest to 1.0 (conventional 1 full unit).
        own_ix  = argmin(abs.(xpg .- 1.0))
        own_amt = xpg[own_ix]   # 1.0 with default grid {0, 0.5, 1.0}

        # ── Case 1: rent (x_ell_new = 0) ────────────────────────────────────
        tc_rent   = tx_cost_v4(p, x_A_prev, x_B_prev, 0.0, 0.0)
        ...
        # ── Case 2: own (x_ell_new = own_amt; x_{ell'} = 0) ────────────────
        xA_own  = ell == LOC_A ? own_amt : 0.0
        xB_own  = ell == LOC_B ? own_amt : 0.0
        ix_A_own = ell == LOC_A ? own_ix : 1   # index for own_amt or 0
        ix_B_own = ell == LOC_B ? own_ix : 1
        tc_own   = tx_cost_v4(p, x_A_prev, x_B_prev, xA_own, xB_own)
        own_res  = w - p.m - own_amt - tc_own
        if own_res > 0.0
            b_lo = -p.ltv_max * own_amt
        ...
        # (x_A, x_B) each chosen from x_prev grid — N_X_PREV^2 pairs total.
        # Budget: c + kappa(x_A_new, ell) + b + s + x_A_new + x_B_new + tx_cost = w
        for (ix_A_new, x_A_new) in enumerate(xpg)
            for (ix_B_new, x_B_new) in enumerate(xpg)
...
function solve_v4(;
    ...
    for t in (t_last - 1):-1:1
        ...
        for (iw, w) in enumerate(grids.w),
            ...
            iell in 1:2,
            ixA_prev in 1:n_xp, ixB_prev in 1:n_xp
...
function summary_v4(result::SolverResult_v4, grids::Grids_v4,
    ...
    # Policy statistics at t=1 initial condition (x_prev=0 for both)
    for (lbl, iell) in [("ellA", LOC_A), ("ellB", LOC_B)]
        f1   = result.feasible[1, :, :, iell, 1, 1]
        xAp  = result.xA_policy[1, :, :, iell, 1, 1]
        xBp  = result.xB_policy[1, :, :, iell, 1, 1]
        if any(f1)
            s["mean_xA_t1_x00_$lbl"]       = mean(xAp[f1])
            s["mean_xB_t1_x00_$lbl"]       = mean(xBp[f1])
            s["xA_gt0_count_t1_x00_$lbl"]  = count(x -> x > 0.0, xAp[f1])
            s["xB_gt0_count_t1_x00_$lbl"]  = count(x -> x > 0.0, xBp[f1])
            s["feasible_count_t1_x00_$lbl"] = count(f1)
        else
            s["mean_xA_t1_x00_$lbl"] = nothing
            s["mean_xB_t1_x00_$lbl"] = nothing
        end
    end
...
# ─────────────────────────────────────────────────────────────────────────────
# Smoke test — no VFI; checks struct init, tx_cost, 6D allocation, shocks
...
function smoke_test_v4()
    ...
    @printf("  tau_sell=%.4f, tau_buy=%.4f (ACTIVE), tau_token=%.4f (ACTIVE)\n",
    ...
    println("  sigma decomp: sqrt($(round(params.sigma_div,digits=4))^2 + " *
            "$(round(params.sigma_iota,digits=4))^2) = " *
            "$(round(sqrt(params.sigma_div^2+params.sigma_iota^2),digits=6)) " *
            "(sigma_h=$(params.sigma_h)) — OK: $check1")
    @assert check1 "sigma decomposition failed"

    # x_prev grid
    grids = build_grids_v4(spec, cfg)
    @printf("  x_prev grid (N=%d, max=%.1f): %s\n",
            length(grids.x_prev), cfg.x_prev_max, string(grids.x_prev))
    @assert length(grids.x_prev) == cfg.n_x_prev "x_prev grid size mismatch"
    @assert grids.x_prev[1] ≈ 0.0 "x_prev_grid[1] must be 0.0"
    @assert grids.x_prev[end] ≈ cfg.x_prev_max "x_prev_grid[end] must equal x_prev_max"

    # tx_cost spot checks
    tc_buy_full = tx_cost_v4(params, 0.0, 0.0, 1.0, 0.0)   # buy x_A from 0 to 1
    @assert abs(tc_buy_full - params.tau_buy * 1.0) < 1e-12 "tx_cost: 0→1 buy"
    tc_sell_full = tx_cost_v4(params, 1.0, 0.5, 0.0, 0.0)  # sell all
    expected_sell = params.tau_token * 1.5
    @assert abs(tc_sell_full - expected_sell) < 1e-12 "tx_cost: sell x_A=1,x_B=0.5 → 0"
    tc_noop = tx_cost_v4(params, 0.5, 0.5, 0.5, 0.5)       # no change
    @assert tc_noop == 0.0 "tx_cost: no change should cost 0"
    tc_mixed = tx_cost_v4(params, 0.0, 1.0, 1.0, 0.5)      # buy A, sell B partial
    expected_mixed = params.tau_buy * 1.0 + params.tau_token * 0.5
    @assert abs(tc_mixed - expected_mixed) < 1e-12 "tx_cost: mixed buy/sell"
    ...
    n_xp   = cfg.n_x_prev
    @printf("  value 6D dims: %s  (T=%d, N_W=%d, N_Z=%d, N_ell=2, N_X_PREV=%d)\n",
    ...
    @assert dims[5] == n_xp                  "x_A_prev dimension wrong"
    @assert dims[6] == n_xp                  "x_B_prev dimension wrong"

    total_pts = prod(dims)
    mem_mb    = total_pts * 8 * 6 / 1024^2  # 6 float64 arrays
    @printf("  6D state total: %d points; policy arrays ~%.1f MB\n", total_pts, mem_mb)
    @assert mem_mb < 500.0 "Memory estimate exceeds 500 MB — reduce grid sizes"

    # Terminal slice
    terminal_slice_v4!(result, params, grids, T)
    @assert all(result.feasible[T, :, :, :, :, :]) "some terminal states infeasible"
    @assert !any(isnan, result.value[T, :, :, :, :, :]) "NaN in terminal slice"
    println("  terminal slice (6D): PASS")
    ...
    @printf("  shock block: %d points (=%d^7), weight_sum=%.8f\n",
    ...
    @assert housing_cost_v4(0.0, 0.0, LOC_A, p, REGIME_E1_2L) == p.rho   # renter at A
    ...
    # x_prev = x_new gives tx_cost = 0 for all grid points
    for xA in grids.x_prev, xB in grids.x_prev
        @assert tx_cost_v4(params, xA, xB, xA, xB) == 0.0 "no-change state has nonzero tx_cost"
    end
    println("  x_prev=x_new zero-cost: PASS")

    # E1_2L own_ix selection
    own_ix = argmin(abs.(grids.x_prev .- 1.0))
    @printf("  E1_2L own_ix=%d, own_amt=%.4f  (should be 1.0 with default grid)\n",
            own_ix, grids.x_prev[own_ix])
...
function main_v4(args::Vector{String}=ARGS)
    ...
    @printf("  6D state  : T×NW×NZ×2×Nxp×Nxp = %d×%d×%d×2×%d×%d\n",
    ...
    @printf("  tx costs  : tau_sell=%.4f (reloc sell-factor), tau_buy=%.4f (ACTIVE), tau_token=%.4f (ACTIVE)\n",
            params.tau_sell, params.tau_buy, params.tau_token)
