...
# Path B Option 1: proper per-period tau_buy on position deltas
#
# Key change from v3: x_A_prev and x_B_prev are explicit state variables.
# Transaction costs charged on position DELTAS each period:
#   tx_cost = tau_buy   * (max(x_A_new - x_A_prev, 0) + max(x_B_new - x_B_prev, 0))
#           + tau_token * (max(x_A_prev - x_A_new, 0) + max(x_B_prev - x_B_new, 0))
#
# x_new choices restricted to x_prev_grid so next-period (ell, xA, xB) state
# is looked up exactly — no interpolation across the x_prev dimensions.
#
# Default grid: N_X_PREV=3 points in [0, 1.0] → {0.0, 0.5, 1.0}.
#
# Regimes:
#   E1_2L — binary x_ell ∈ {0, 1.0}, x_{ell'} = 0; forced sell on reloc, x_prev resets to 0
#   E2_2L — x_A, x_B ∈ x_prev_grid; tokens portable across moves (x_prev carries through)
#
# Why this resurrects the hedge: a household at A pre-buying x_B_prev > 0 pays
# tau_buy only on the INCREMENT above x_B_prev when relocating to B — saving
# tau_buy * x_B_prev compared to E1_2L which starts from 0 at every relocation.
#
...
function regime_from_env_v4()
    ...
    else; error("Unknown REGIME='$name'. Use E1_2L or E2_2L."); end
end
...
struct GridSpec_v4
    ...
    x_prev_max::Float64  # upper bound (default 1.0; includes 1.0 for E1_2L compat)
end
...
struct Grids_v4
    w::Vector{Float64}
    z::Vector{Float64}
    x_prev::Vector{Float64}  # discrete x_prev grid; x_new choices come from this
end
...
mutable struct SolverResult_v4
    ...
    xA_policy::Array{Float64,6}   # value of x_A_new chosen
    xB_policy::Array{Float64,6}   # value of x_B_new chosen
    feasible::BitArray{6}
...
function build_grids_v4(s::GridSpec_v4)
    ...
    return Grids_v4(build_w_grid_v4(s), build_z_grid_v4(s), x_prev)
end

# Index of 0.0 in x_prev grid (always first element).
ix_x0_v4(grids::Grids_v4) = 1

# Index of 1.0 in x_prev grid; error if not found within 1e-6.
function ix_one_v4(grids::Grids_v4)
    idx = argmin(abs.(grids.x_prev .- 1.0))
    abs(grids.x_prev[idx] - 1.0) > 1e-4 &&
        @warn "x_prev grid nearest to 1.0 is $(grids.x_prev[idx]); E1_2L own case approximate"
    return idx
end
...
function build_shock_block_v4(p::ModelParams_v4, cfg::SolveConfig_v4)
    ...
    u  = Vector{Float64}(undef, total); ep = Vector{Float64}(undef, total)
                    ...
                    for (i5, nh) in enumerate(nodes)
                        ...
                        for (i6, nu) in enumerate(nodes)
                            ...
                            for (i7, ne) in enumerate(nodes)
                                ...
                                hp[idx] = hp_val; u[idx]  = u_val;  ep[idx] = ep_val
    ...
    @assert idx == total
    return ShockBlock_v4(rs, ra, rb, hp, u, ep, wt)
end
...
# Transaction cost on position deltas (the core v4 mechanism).
...
@inline function next_wealth_v4(p::ModelParams_v4,
    ...
    return (b * rate_b + s * rs_next +
            x_A_new * ra_next * sf_A +
            x_B_new * rb_next * sf_B) / hp_next + y_next
end
...
# ─────────────────────────────────────────────────────────────────────────────
# Continuation value — integrates quadrature + relocation shock
# next_value: (n_w, n_z, 2, n_xp, n_xp) slice at t+1
# ix_A_new, ix_B_new: chosen grid indices (exact next-period x_prev state)
# ix_x0: index of 0.0 in x_prev grid (used for E1_2L relocation reset)
# ─────────────────────────────────────────────────────────────────────────────

function continuation_value_v4(
    p::ModelParams_v4, grids::Grids_v4, shock::ShockBlock_v4,
    f_profile::Vector{Float64},
    next_value::AbstractArray{Float64,5},
    t::Int, z::Float64, ell::Int,
    b::Float64, s::Float64,
    x_A_new::Float64, x_B_new::Float64,
    ix_A_new::Int, ix_B_new::Int,
    regime::Int, ix_x0::Int,
)
    ...
    # E2_2L: tokens portable — carry (ix_A_new, ix_B_new) regardless of relocation.
    # E1_2L stay:   carry (ix_A_new, ix_B_new) — ix_B_new = ix_x0 always (at A, xB=0).
    # E1_2L reloc:  sold location resets to ix_x0; non-occupied also ix_x0.
    ix_A_stay = ix_A_new;   ix_B_stay = ix_B_new
    if regime == REGIME_E1_2L
        # On relocation from A: A was sold → ix_A_reloc = ix_x0; B never held → ix_x0.
        # On relocation from B: B was sold → ix_B_reloc = ix_x0; A never held → ix_x0.
        ix_A_reloc = ix_x0;  ix_B_reloc = ix_x0
    else
        ix_A_reloc = ix_A_new;  ix_B_reloc = ix_B_new
    end
    ...
    @inbounds for q in eachindex(shock.weights)
        ...
        v_stay  = interp_bilinear_v4(
            view(next_value, :, :, ell,     ix_A_stay,  ix_B_stay),
            grids.w, grids.z, w_stay, z_next)
        v_reloc = interp_bilinear_v4(
            view(next_value, :, :, ell_alt, ix_A_reloc, ix_B_reloc),
...
# ix_A_prev, ix_B_prev: current state indices into grids.x_prev
# Returns (best_v, c, b, s, xA_val, xB_val, feasible)
# ─────────────────────────────────────────────────────────────────────────────
...
function solve_state_v4(
    ...
    regime::Int, ix_x0::Int, ix_one::Int,
)
    ...
    if regime == REGIME_E1_2L
        ...
        # In E1_2L, x_{ell'} is always 0 by admissibility regardless of ell.
        ix_A_new = ix_x0;  ix_B_new = ix_x0
        tx = tx_cost_v4(x_A_prev, x_B_prev, 0.0, 0.0, p.tau_buy, p.tau_token)
        res_rent = w - p.rho - tx
        if res_rent > 0.0
                ...
                for s in candidate_grid_v4(max_s, na)
                    ...
                    v = utility_crra_v4(c, p.gamma) +
                        p.beta * continuation_value_v4(
                            p, grids, shock, f_profile, next_value,
                            t, z, ell, b, s, 0.0, 0.0, ix_A_new, ix_B_new, regime, ix_x0)
                    if v > best_v
        ...
        # ── Case 2: own at current location (x_ell_new=1.0) ─────────────────
        xA_own  = ell == LOC_A ? 1.0 : 0.0
        xB_own  = ell == LOC_B ? 1.0 : 0.0
        ix_A_own = ell == LOC_A ? ix_one : ix_x0
        ix_B_own = ell == LOC_B ? ix_one : ix_x0
        tx_own   = tx_cost_v4(x_A_prev, x_B_prev, xA_own, xB_own, p.tau_buy, p.tau_token)
        # Budget: c + m + 1.0 (unit purchase) + b + s + tx_own = w
        own_budget = w - p.m - 1.0 - tx_own
        if own_budget > 0.0
            b_lo = -p.ltv_max * 1.0
            b_cands = p.ltv_max > 0.0 ?
                collect(range(b_lo, max(own_budget, b_lo + 1e-6); length=na)) :
                candidate_grid_v4(own_budget, na)
            for b in b_cands
                b < b_lo && continue
                max_s = max(own_budget - b, 0.0)
                for s in candidate_grid_v4(max_s, na)
                    c = own_budget - b - s;  c <= 0.0 && continue
                    v = utility_crra_v4(c, p.gamma) +
                        p.beta * continuation_value_v4(
                            p, grids, shock, f_profile, next_value,
                            t, z, ell, b, s, xA_own, xB_own, ix_A_own, ix_B_own, regime, ix_x0)
                    if v > best_v
                        best_v, best_c, best_b, best_s = v, c, b, s
                    ...
                    for s in candidate_grid_v4(max_s, na)
                        ...
                        v = utility_crra_v4(c, p.gamma) +
                            p.beta * continuation_value_v4(
                                ...
                                ix_A_new, ix_B_new, regime, ix_x0)
                        if v > best_v
                            best_v, best_c, best_b, best_s = v, c, b, s
...
function solve_v4(;
    ...
    ix_x0     = ix_x0_v4(grids)
    ix_one    = ix_one_v4(grids)
    n_xp      = length(grids.x_prev)
    ...
    for t in (t_last - 1):-1:1
        ...
        for (iw, w) in enumerate(grids.w),
            ...
            v, c, b, s, xA, xB, ok = solve_state_v4(
                ...
                next_slice, t, w, z, iell, ix_A_prev, ix_B_prev,
                regime, ix_x0, ix_one,
            )
...
# ─────────────────────────────────────────────────────────────────────────────
# Summary — operates on the initial state (t=1, ix_A_prev=ix_x0, ix_B_prev=ix_x0)
# ─────────────────────────────────────────────────────────────────────────────
...
function summary_v4(result::SolverResult_v4, grids::Grids_v4,
    ...
    # Report at initial state: t=1, x_A_prev=0, x_B_prev=0 (household enters with no prior holdings)
    ix_x0 = ix_x0_v4(grids)
    iw_mid = max(1, div(length(grids.w), 2))
    iz_mid = max(1, div(length(grids.z), 2))
    s["V_t1_midpoint_ellA_x0"] = result.value[1, iw_mid, iz_mid, LOC_A, ix_x0, ix_x0]
    s["V_t1_midpoint_ellB_x0"] = result.value[1, iw_mid, iz_mid, LOC_B, ix_x0, ix_x0]

    for (lbl, iell) in [("ellA", LOC_A), ("ellB", LOC_B)]
        v1  = view(result.value,     1, :, :, iell, ix_x0, ix_x0)
        f1  = view(result.feasible,  1, :, :, iell, ix_x0, ix_x0)
        xAp = view(result.xA_policy, 1, :, :, iell, ix_x0, ix_x0)
        xBp = view(result.xB_policy, 1, :, :, iell, ix_x0, ix_x0)
...
# ─────────────────────────────────────────────────────────────────────────────
# Smoke test — no VFI; checks struct allocation, tx_cost, and state consistency
...
function smoke_test_v4()
    ...
    # sigma decomposition
    sd_check = abs(sqrt(params.sigma_div^2 + params.sigma_iota^2) - params.sigma_h) < 1e-8
    println("  sigma decomp OK: $sd_check");  @assert sd_check

    spec  = default_grids_v4(small=true)
    grids = build_grids_v4(spec)
    @printf("  grid: N_W=%d, N_Z=%d, N_X_PREV=%d, x_prev=%s\n",
            spec.n_w, spec.n_z, spec.n_x_prev, string(round.(grids.x_prev; digits=3)))
    @assert length(grids.w) == spec.n_w
    @assert length(grids.z) == spec.n_z
    @assert length(grids.x_prev) == spec.n_x_prev

    ix_x0 = ix_x0_v4(grids);  ix_one = ix_one_v4(grids)
    @printf("  ix_x0=%d (x=%.2f), ix_one=%d (x=%.2f)\n",
            ix_x0, grids.x_prev[ix_x0], ix_one, grids.x_prev[ix_one])
    @assert grids.x_prev[ix_x0] == 0.0

    # tx_cost formula checks
    @assert abs(tx_cost_v4(0.0, 0.0, 0.5, 0.0, 0.025, 0.01) - 0.025*0.5) < 1e-12  "buy check"
    @assert abs(tx_cost_v4(0.5, 0.0, 0.0, 0.0, 0.025, 0.01) - 0.01*0.5)  < 1e-12  "sell check"
    @assert tx_cost_v4(0.5, 0.5, 0.5, 0.5, 0.025, 0.01) == 0.0                     "hold check"
    @assert abs(tx_cost_v4(0.0, 0.0, 0.5, 0.5, 0.025, 0.01) - 0.025*1.0) < 1e-12  "buy both"
    # Key hedge savings: pre-held x_B_prev=0.5 vs 0.0 when buying to 1.0
    save = tx_cost_v4(0.0, 0.0, 0.0, 1.0, 0.025, 0.01) -
           tx_cost_v4(0.0, 0.5, 0.0, 1.0, 0.025, 0.01)
    @assert abs(save - 0.025*0.5) < 1e-12  "hedge savings check"
    @printf("  tx_cost checks: PASS  (hedge saving = %.4f for x_B_prev=0.5 vs 0.0)\n", save)
    ...
    # p_relocate boundary
    @assert p_relocate_v4(params, 1)  == params.p_relocate_working
    @assert p_relocate_v4(params, 42) == params.p_relocate_retired
...
function main_v4(args::Vector{String}=ARGS)
    ...
    @printf("  x_prev_grid: %s\n", string(round.(grids_tmp.x_prev; digits=3)))
