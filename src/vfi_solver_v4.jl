#!/usr/bin/env julia
# vfi_solver_v4.jl — Option 1: 6D state (t, w, z, ell, x_A_prev, x_B_prev)
#
# Proper tau_buy hedge mechanism: pre-holding x_B at ell=A charges tau_buy
# incrementally each period, avoiding a lump-sum tau_buy on forced purchase
# after relocation. A household at A who anticipates relocation to B can
# accumulate x_B cheaply over multiple periods.
#
# State:    (t, w, z, ell, x_A_prev, x_B_prev) — 6D
# Controls: regime-dependent:
#   E0      — (c, b, s)                    rent-only
#   E1_2L   — (c, b, s, x_ell_new ∈ {0,1}) binary at ell; x_ell'=0 always
#   E2_2L   — (c, b, s, x_A_new, x_B_new)  from x_prev_grid × x_prev_grid
#
# Transaction costs (per period):
#   delta_A  = x_A_new - x_A_prev
#   delta_B  = x_B_new - x_B_prev
#   E2_2L:  tx = tau_buy * (max(δA,0) + max(δB,0)) + tau_token * (max(-δA,0) + max(-δB,0))
#   E1_2L:  tx = tau_buy * (max(δ,0)) + tau_sell  * (max(-δ,0))   (voluntary sell at sell cost)
#   Forced relocation sell (E1_2L) still applied via sell_factor in wealth transition.
#
# Relocation x_prev transition:
#   E2_2L: x_prev_next = x_new (tokens portable — same for stay and reloc branches)
#   E1_2L: stay  → x_prev_next = (x_ell_new, 0)
#          reloc → x_prev_next = (0, 0) (forced sale, nothing carried to new location)
#
# Choices restricted to x_prev_grid values → next-period x_prev index is exact (no
# interpolation in x_prev dimension).
#
# Default grids: N_X_PREV=3 {0.0, 0.75, 1.5}, N_W=15, N_Z=5
# Compute: ~4.6x v3 per regime (~2-3h wall on server1 single thread).
#
# Run:    REGIME=E1_2L julia src/vfi_solver_v4.jl
#         REGIME=E2_2L julia src/vfi_solver_v4.jl
# Smoke:  julia src/vfi_solver_v4.jl --smoke-test

using Dates, Printf, Serialization, Statistics, JSON3

const NEG_INF = -1.0e18

const REGIME_E0    = 1
const REGIME_E1_2L = 2
const REGIME_E2_2L = 3
const LOC_A = 1
const LOC_B = 2

regime_name_v4(r::Int) =
    r == REGIME_E0 ? "E0" : r == REGIME_E1_2L ? "E1_2L" : "E2_2L"

function regime_from_env_v4()
    name = get(ENV, "REGIME", "E2_2L")
    if     name == "E0";    return REGIME_E0
    elseif name == "E1_2L"; return REGIME_E1_2L
    elseif name == "E2_2L"; return REGIME_E2_2L
    else;  error("Unknown REGIME='$name'. Use E0, E1_2L, or E2_2L.")
    end
end

# ─────────────────────────────────────────────────────────────────────────────
# Parameters (identical set to v3)
# ─────────────────────────────────────────────────────────────────────────────

struct ModelParams_v4
    gamma::Float64;  beta::Float64;   rf::Float64
    mu_s::Float64;   sigma_s::Float64
    mu_h::Float64;   sigma_h::Float64; g_h::Float64; sigma_xi::Float64
    rho::Float64;    m::Float64
    sigma_u::Float64; sigma_eps::Float64
    lambda_ret::Float64
    age0::Int; retire_age::Int; terminal_age::Int
    sigma_div::Float64; sigma_iota::Float64; rho_AB::Float64
    p_relocate_working::Float64; p_relocate_retired::Float64
    tau_sell::Float64; tau_buy::Float64; tau_token::Float64
    ltv_max::Float64; r_mort_premium::Float64
end

struct GridSpec_v4
    n_w::Int; w_min::Float64; w_max::Float64
    n_z::Int; z_min::Float64; z_max::Float64
    n_x_prev::Int        # coarse x_prev grid size (default 3)
    x_prev_max::Float64  # upper bound of x_prev grid (default 1.5)
end

struct SolveConfig_v4
    asset_grid_size::Int
    quadrature_nodes::Int
    small_grid_mode::Bool
    save_path::Union{Nothing,String}
end

struct ShockBlock_v4
    rs::Vector{Float64}; ra::Vector{Float64}; rb::Vector{Float64}
    hp::Vector{Float64}; u::Vector{Float64};  eps::Vector{Float64}
    weights::Vector{Float64}
end

# 6D: (t, iw, iz, iell, ixA_prev, ixB_prev)
mutable struct SolverResult_v4
    value::Array{Float64,6}
    c_policy::Array{Float64,6}
    b_policy::Array{Float64,6}
    s_policy::Array{Float64,6}
    xA_policy::Array{Float64,6}
    xB_policy::Array{Float64,6}
    feasible::BitArray{6}
    metadata::Dict{String,Any}
end

# ─────────────────────────────────────────────────────────────────────────────
# Default parameters and grids
# ─────────────────────────────────────────────────────────────────────────────

function default_params_v4()
    gamma         = parse(Float64, get(ENV, "GAMMA",           "5.0"))
    rf            = parse(Float64, get(ENV, "RF",              "1.02"))
    eq_prem       = parse(Float64, get(ENV, "EQUITY_PREMIUM",  "0.04"))
    sigma_s       = parse(Float64, get(ENV, "SIGMA_S",         "0.157"))
    g_h           = parse(Float64, get(ENV, "G_H",             "0.016"))
    sigma_h       = parse(Float64, get(ENV, "SIGMA_H",         "0.115"))
    sigma_xi      = parse(Float64, get(ENV, "SIGMA_XI",        string(sigma_h)))
    mu_s          = log(rf + eq_prem) - 0.5 * sigma_s^2
    mu_h_default  = log(1.0 + g_h) - 0.5 * sigma_h^2
    mu_h          = parse(Float64, get(ENV, "MU_H",            string(mu_h_default)))
    sigma_div     = parse(Float64, get(ENV, "SIGMA_DIV",       "0.10"))
    sigma_div >= sigma_h && error("sigma_div ($sigma_div) must be < sigma_h ($sigma_h)")
    sigma_iota    = sqrt(sigma_h^2 - sigma_div^2)
    rho_AB        = clamp(parse(Float64, get(ENV, "RHO_AB",    "0.50")),
                          -1.0 + 1e-8, 1.0 - 1e-8)
    return ModelParams_v4(
        gamma,
        parse(Float64, get(ENV, "BETA",               "0.96")),
        rf, mu_s, sigma_s, mu_h, sigma_h, g_h, sigma_xi,
        parse(Float64, get(ENV, "RHO",                "0.05")),
        parse(Float64, get(ENV, "M",                  "0.01")),
        sqrt(parse(Float64, get(ENV, "SIGMA_U2",      "0.0106"))),
        sqrt(parse(Float64, get(ENV, "SIGMA_EPS2",    "0.0738"))),
        parse(Float64, get(ENV, "LAMBDA_RET",         "0.65")),
        parse(Int,     get(ENV, "AGE0",               "25")),
        parse(Int,     get(ENV, "RETIRE_AGE",         "65")),
        parse(Int,     get(ENV, "TERMINAL_AGE",       "80")),
        sigma_div, sigma_iota, rho_AB,
        parse(Float64, get(ENV, "P_RELOCATE_WORKING", "0.06")),
        parse(Float64, get(ENV, "P_RELOCATE_RETIRED", "0.02")),
        parse(Float64, get(ENV, "TAU_SELL",           "0.06")),
        parse(Float64, get(ENV, "TAU_BUY",            "0.025")),
        parse(Float64, get(ENV, "TAU_TOKEN",          "0.01")),
        parse(Float64, get(ENV, "LTV_MAX",            "0.0")),
        parse(Float64, get(ENV, "R_MORT_PREMIUM",     "0.005")),
    )
end

function default_grids_v4(; small::Bool=true)
    if small
        return GridSpec_v4(
            parse(Int,     get(ENV, "N_W",       "15")),
            parse(Float64, get(ENV, "W_MIN",     "0.02")),
            parse(Float64, get(ENV, "W_MAX",     "12.0")),
            parse(Int,     get(ENV, "N_Z",       "5")),
            parse(Float64, get(ENV, "Z_MIN",     "0.15")),
            parse(Float64, get(ENV, "Z_MAX",     "3.5")),
            parse(Int,     get(ENV, "N_X_PREV",  "3")),
            parse(Float64, get(ENV, "X_PREV_MAX","1.5")),
        )
    else
        return GridSpec_v4(
            parse(Int,     get(ENV, "N_W",       "40")),
            parse(Float64, get(ENV, "W_MIN",     "0.001")),
            parse(Float64, get(ENV, "W_MAX",     "50.0")),
            parse(Int,     get(ENV, "N_Z",       "9")),
            parse(Float64, get(ENV, "Z_MIN",     "0.05")),
            parse(Float64, get(ENV, "Z_MAX",     "8.0")),
            parse(Int,     get(ENV, "N_X_PREV",  "5")),
            parse(Float64, get(ENV, "X_PREV_MAX","2.0")),
        )
    end
end

function default_config_v4(; small::Bool=true)
    return SolveConfig_v4(
        parse(Int, get(ENV, "ASSET_GRID_SIZE", small ? "9"  : "15")),
        parse(Int, get(ENV, "GH_NODES",        "3")),
        small,
        get(ENV, "SAVE_PATH", nothing),
    )
end

build_w_grid_v4(s::GridSpec_v4) =
    collect(s.w_min .+ (s.w_max - s.w_min) .* (range(0.0, 1.0; length=s.n_w) .^ 3.0))
build_z_grid_v4(s::GridSpec_v4) =
    collect(exp.(range(log(s.z_min), log(s.z_max); length=s.n_z)))
build_xprev_grid_v4(s::GridSpec_v4) =
    collect(range(0.0, s.x_prev_max; length=s.n_x_prev))

# ─────────────────────────────────────────────────────────────────────────────
# Gauss-Hermite quadrature (copied from v3)
# ─────────────────────────────────────────────────────────────────────────────

function gh_rule_v4(n::Int)
    if n == 3
        nodes   = [-sqrt(3.0/2.0), 0.0, sqrt(3.0/2.0)]
        weights = [sqrt(π)/6.0, 2.0*sqrt(π)/3.0, sqrt(π)/6.0]
    elseif n == 5
        nodes   = [-2.0201828704560856, -0.9585724646138185, 0.0,
                    0.9585724646138185,  2.0201828704560856]
        weights = [0.01995324205904591, 0.39361932315224116, 0.9453087204829419,
                   0.39361932315224116, 0.01995324205904591]
    else
        error("Only 3 or 5 GH nodes supported.")
    end
    return nodes, weights ./ sqrt(π)
end

function build_shock_block_v4(p::ModelParams_v4, cfg::SolveConfig_v4)
    nodes, weights = gh_rule_v4(cfg.quadrature_nodes)
    n     = cfg.quadrature_nodes
    total = n^7
    rs  = Vector{Float64}(undef, total);  ra  = Vector{Float64}(undef, total)
    rb  = Vector{Float64}(undef, total);  hp  = Vector{Float64}(undef, total)
    u_s = Vector{Float64}(undef, total);  eps = Vector{Float64}(undef, total)
    wts = Vector{Float64}(undef, total)
    sqrt1mr2 = sqrt(max(1.0 - p.rho_AB^2, 0.0))
    idx = 0
    for (i1,ns) in enumerate(nodes)
        eta_s = sqrt(2.0)*p.sigma_s*ns;  rs_val = exp(p.mu_s + eta_s)
        for (i2,nd) in enumerate(nodes)
            eta_div = sqrt(2.0)*p.sigma_div*nd
            for (i3,nA) in enumerate(nodes)
                iota_A = sqrt(2.0)*p.sigma_iota*nA
                ra_val = exp(p.mu_h + eta_div + iota_A)
                for (i4,nB) in enumerate(nodes)
                    iota_B = p.rho_AB*iota_A + sqrt1mr2*sqrt(2.0)*p.sigma_iota*nB
                    rb_val = exp(p.mu_h + eta_div + iota_B)
                    for (i5,nh) in enumerate(nodes)
                        xi = sqrt(2.0)*p.sigma_xi*nh;  hp_val = exp(p.g_h + xi)
                        for (i6,nu) in enumerate(nodes)
                            u_val = sqrt(2.0)*p.sigma_u*nu
                            for (i7,ne) in enumerate(nodes)
                                eps_val = sqrt(2.0)*p.sigma_eps*ne
                                idx += 1
                                rs[idx]=rs_val; ra[idx]=ra_val; rb[idx]=rb_val
                                hp[idx]=hp_val; u_s[idx]=u_val; eps[idx]=eps_val
                                wts[idx] = (weights[i1]*weights[i2]*weights[i3]*
                                            weights[i4]*weights[i5]*weights[i6]*weights[i7])
                            end
                        end
                    end
                end
            end
        end
    end
    @assert idx == total
    return ShockBlock_v4(rs, ra, rb, hp, u_s, eps, wts)
end

# ─────────────────────────────────────────────────────────────────────────────
# Model mechanics
# ─────────────────────────────────────────────────────────────────────────────

@inline utility_crra_v4(c::Float64, γ::Float64) =
    c <= 0.0 ? NEG_INF :
    (isapprox(γ, 1.0; atol=1e-12) ? log(c) : c^(1.0 - γ) / (1.0 - γ))

@inline function p_relocate_v4(p::ModelParams_v4, t::Int)::Float64
    age = p.age0 + t - 1
    return age <= p.retire_age ? p.p_relocate_working : p.p_relocate_retired
end

# Housing cost rule — same as v3 (fixed kappa: only occupied-unit token saves rent)
@inline function housing_cost_v4(x_A::Float64, x_B::Float64, ell::Int,
                                   p::ModelParams_v4, regime::Int)::Float64
    if regime == REGIME_E0
        return p.rho
    elseif regime == REGIME_E1_2L
        x_ell = ell == LOC_A ? x_A : x_B
        return x_ell >= 1.0 ? p.m : p.rho
    else  # E2_2L
        x_ell_local = ell == LOC_A ? x_A : x_B
        return p.rho - x_ell_local * (p.rho - p.m)
    end
end

function income_profile_v4(p::ModelParams_v4)
    ages = p.age0:p.terminal_age
    f = Vector{Float64}(undef, length(ages))
    for (i, a) in enumerate(ages)
        aa = a / 10.0
        f[i] = -2.17042 + 0.16818*aa - 0.03230*aa^2 + 0.00200*aa^3
    end
    return f
end

function next_income_state_v4(p::ModelParams_v4, f::Vector{Float64}, t::Int, z::Float64,
                                hp_next::Float64, u::Float64, eps::Float64)
    next_t   = t + 1
    next_age = p.age0 + next_t - 1
    if next_age <= p.retire_age
        df     = f[next_t] - f[t]
        z_next = z * exp(df + u) / hp_next
        y_next = z_next * exp(eps)
    elseif p.age0 + t - 1 <= p.retire_age
        z_next = p.lambda_ret * z / hp_next
        y_next = z_next
    else
        z_next = z / hp_next
        y_next = z_next
    end
    return z_next, y_next
end

@inline function next_wealth_v4(p::ModelParams_v4,
                                  b::Float64, s::Float64,
                                  x_A::Float64, x_B::Float64,
                                  hp_next::Float64, rs_next::Float64,
                                  ra_next::Float64, rb_next::Float64,
                                  sfA::Float64, sfB::Float64,
                                  y_next::Float64)
    rate_b = b >= 0.0 ? p.rf : (p.rf + p.r_mort_premium)
    return (b*rate_b + s*rs_next + x_A*ra_next*sfA + x_B*rb_next*sfB) / hp_next + y_next
end

function interp_bilinear_v4(vals::AbstractMatrix{Float64},
                              w_grid::Vector{Float64}, z_grid::Vector{Float64},
                              w::Float64, z::Float64)
    n_w = length(w_grid); n_z = length(z_grid)
    if w <= w_grid[1];       i_w = 1;       f_w = 0.0
    elseif w >= w_grid[end]; i_w = n_w - 1; f_w = 1.0
    else
        i_w = clamp(searchsortedlast(w_grid, w), 1, n_w - 1)
        f_w = (w - w_grid[i_w]) / (w_grid[i_w+1] - w_grid[i_w])
    end
    if z <= z_grid[1];       i_z = 1;       f_z = 0.0
    elseif z >= z_grid[end]; i_z = n_z - 1; f_z = 1.0
    else
        i_z = clamp(searchsortedlast(z_grid, z), 1, n_z - 1)
        f_z = (z - z_grid[i_z]) / (z_grid[i_z+1] - z_grid[i_z])
    end
    v11 = vals[i_w,i_z]; v21 = vals[i_w+1,i_z]
    v12 = vals[i_w,i_z+1]; v22 = vals[i_w+1,i_z+1]
    return (1.0-f_w)*(1.0-f_z)*v11 + f_w*(1.0-f_z)*v21 +
           (1.0-f_w)*f_z*v12    + f_w*f_z*v22
end

# ─────────────────────────────────────────────────────────────────────────────
# Continuation value — 6D lookup
#
# next_slice: view of value[t+1,:,:,:,:,:] → shape (n_w, n_z, 2, n_xA_prev, n_xB_prev)
#
# ix_A_next_stay / ix_B_next_stay:  x_prev indices if household STAYS at ell
# ix_A_next_reloc / ix_B_next_reloc: x_prev indices if household RELOCATES to ell_alt
#   E2_2L: both are the same (tokens portable across moves)
#   E1_2L: reloc indices = (1,1) [x=0; forced sale leaves no holdings]
# ─────────────────────────────────────────────────────────────────────────────

function continuation_value_v4(
    p::ModelParams_v4,
    w_grid::Vector{Float64}, z_grid::Vector{Float64},
    shock::ShockBlock_v4, f_profile::Vector{Float64},
    next_slice::AbstractArray{Float64,5},
    t::Int, z::Float64, ell::Int,
    b::Float64, s::Float64, x_A::Float64, x_B::Float64,
    ix_A_next_stay::Int,  ix_B_next_stay::Int,
    ix_A_next_reloc::Int, ix_B_next_reloc::Int,
    regime::Int,
)
    p_reloc = p_relocate_v4(p, t)
    ell_alt = ell == LOC_A ? LOC_B : LOC_A

    # Sell factors for forced relocation in E1_2L
    sfA_stay = sfB_stay = sfA_reloc = sfB_reloc = 1.0
    if regime == REGIME_E1_2L
        if ell == LOC_A; sfA_reloc = 1.0 - p.tau_sell
        else;            sfB_reloc = 1.0 - p.tau_sell
        end
    end

    ev = 0.0
    @inbounds for q in eachindex(shock.weights)
        z_next, y_next = next_income_state_v4(p, f_profile, t, z,
                                               shock.hp[q], shock.u[q], shock.eps[q])
        hp_scale = exp((1.0 - p.gamma) * log(shock.hp[q]))

        w_stay  = next_wealth_v4(p, b, s, x_A, x_B, shock.hp[q], shock.rs[q],
                                  shock.ra[q], shock.rb[q],
                                  sfA_stay, sfB_stay, y_next)
        w_reloc = next_wealth_v4(p, b, s, x_A, x_B, shock.hp[q], shock.rs[q],
                                  shock.ra[q], shock.rb[q],
                                  sfA_reloc, sfB_reloc, y_next)

        v_stay  = interp_bilinear_v4(
            view(next_slice, :, :, ell,     ix_A_next_stay,  ix_B_next_stay),
            w_grid, z_grid, w_stay, z_next)
        v_reloc = interp_bilinear_v4(
            view(next_slice, :, :, ell_alt, ix_A_next_reloc, ix_B_next_reloc),
            w_grid, z_grid, w_reloc, z_next)

        ev += shock.weights[q] * hp_scale *
              ((1.0 - p_reloc) * v_stay + p_reloc * v_reloc)
    end
    return ev
end

# ─────────────────────────────────────────────────────────────────────────────
# State solver
# ─────────────────────────────────────────────────────────────────────────────

cand_grid(total::Float64, n::Int) =
    total <= 0.0 ? [0.0] : collect(range(0.0, total; length=n))

function solve_state_v4(
    p::ModelParams_v4,
    w_grid::Vector{Float64}, z_grid::Vector{Float64},
    x_prev_grid::Vector{Float64},
    cfg::SolveConfig_v4, shock::ShockBlock_v4, f_profile::Vector{Float64},
    next_slice::AbstractArray{Float64,5},
    t::Int, w::Float64, z::Float64, ell::Int,
    ix_A_prev::Int, ix_B_prev::Int,
    regime::Int,
)
    best_v  = NEG_INF
    best_c = best_b = best_s = best_xA = best_xB = 0.0
    na      = cfg.asset_grid_size

    x_A_prev = x_prev_grid[ix_A_prev]
    x_B_prev = x_prev_grid[ix_B_prev]

    # Index of 0.0 in x_prev_grid (always element 1 by construction)
    ix_0 = 1

    if regime == REGIME_E0
        res = w - p.rho
        res <= 0.0 && return best_v, best_c, best_b, best_s, 0.0, 0.0, false
        for b in cand_grid(res, na)
            max_s = max(res - b, 0.0)
            for s in cand_grid(max_s, na)
                c = res - b - s
                c <= 0.0 && continue
                cv = continuation_value_v4(p, w_grid, z_grid, shock, f_profile,
                                            next_slice, t, z, ell, b, s, 0.0, 0.0,
                                            ix_0, ix_0, ix_0, ix_0, regime)
                v  = utility_crra_v4(c, p.gamma) + p.beta * cv
                if v > best_v
                    best_v, best_c, best_b, best_s, best_xA, best_xB = v, c, b, s, 0.0, 0.0
                end
            end
        end

    elseif regime == REGIME_E1_2L
        # x_ell_new ∈ {0, 1} at current location; x_ell'_new = 0 always.
        # tx cost: tau_buy on positive delta (buying), tau_sell on negative delta (voluntary sell).
        # Forced relocation sell is handled via sell_factor in wealth transition.

        # Index of 1.0 (or nearest grid point to 1.0)
        ix_1 = argmin(abs.(x_prev_grid .- 1.0))

        # ── Case 1: rent (x_ell_new = 0) ────────────────────────────────────
        # Selling any prior holdings at tau_sell (voluntary liquidation).
        tx_rent = p.tau_sell * (x_A_prev + x_B_prev)
        res_rent = w - p.rho - tx_rent
        if res_rent > 0.0
            for b in cand_grid(res_rent, na)
                max_s = max(res_rent - b, 0.0)
                for s in cand_grid(max_s, na)
                    c = res_rent - b - s
                    c <= 0.0 && continue
                    cv = continuation_value_v4(p, w_grid, z_grid, shock, f_profile,
                                               next_slice, t, z, ell, b, s, 0.0, 0.0,
                                               ix_0, ix_0, ix_0, ix_0, regime)
                    v  = utility_crra_v4(c, p.gamma) + p.beta * cv
                    if v > best_v
                        best_v, best_c, best_b, best_s, best_xA, best_xB = v, c, b, s, 0.0, 0.0
                    end
                end
            end
        end

        # ── Case 2: own (x_ell_new = 1) ─────────────────────────────────────
        if w > 1.0 + p.m
            xA_own = ell == LOC_A ? 1.0 : 0.0
            xB_own = ell == LOC_B ? 1.0 : 0.0
            dA = xA_own - x_A_prev
            dB = xB_own - x_B_prev
            tx_own = (p.tau_buy  * (max(dA, 0.0) + max(dB, 0.0)) +
                      p.tau_sell * (max(-dA, 0.0) + max(-dB, 0.0)))
            res_own = w - p.m - 1.0 - tx_own
            if res_own > 0.0
                b_lo    = -p.ltv_max * 1.0
                b_cands = p.ltv_max > 0.0 ?
                    collect(range(b_lo, max(res_own, b_lo + 1e-6); length=na)) :
                    cand_grid(res_own, na)
                # Stay: carry forward (xA_own, xB_own) → indices
                ix_A_ns = ell == LOC_A ? ix_1 : ix_0
                ix_B_ns = ell == LOC_B ? ix_1 : ix_0
                # Reloc: forced sell → (0, 0) at new location
                ix_A_nr = ix_0;  ix_B_nr = ix_0
                for b in b_cands
                    b < b_lo && continue
                    max_s = max(res_own - b, 0.0)
                    for s in cand_grid(max_s, na)
                        c = res_own - b - s
                        c <= 0.0 && continue
                        cv = continuation_value_v4(p, w_grid, z_grid, shock, f_profile,
                                                   next_slice, t, z, ell, b, s,
                                                   xA_own, xB_own,
                                                   ix_A_ns, ix_B_ns, ix_A_nr, ix_B_nr,
                                                   regime)
                        v  = utility_crra_v4(c, p.gamma) + p.beta * cv
                        if v > best_v
                            best_v,best_c,best_b,best_s,best_xA,best_xB = v,c,b,s,xA_own,xB_own
                        end
                    end
                end
            end
        end

    else  # REGIME_E2_2L
        # Choices: (x_A_new, x_B_new) from x_prev_grid × x_prev_grid.
        # Restricting to grid values ensures next-period x_prev index is exact.
        # tx cost: tau_buy on positive delta, tau_token on negative delta (token transfer).
        # Tokens are portable: stay and reloc branches share same x_prev_next indices.
        for (ix_A_ns, x_A_new) in enumerate(x_prev_grid),
            (ix_B_ns, x_B_new) in enumerate(x_prev_grid)

            dA = x_A_new - x_A_prev
            dB = x_B_new - x_B_prev
            tx = (p.tau_buy   * (max(dA, 0.0) + max(dB, 0.0)) +
                  p.tau_token * (max(-dA, 0.0) + max(-dB, 0.0)))

            kappa = housing_cost_v4(x_A_new, x_B_new, ell, p, REGIME_E2_2L)
            res   = w - kappa - x_A_new - x_B_new - tx
            res <= 0.0 && continue

            x_ell = ell == LOC_A ? x_A_new : x_B_new
            b_lo  = -p.ltv_max * x_ell
            b_cands = (p.ltv_max > 0.0 && x_ell > 0.0) ?
                collect(range(b_lo, max(res, b_lo + 1e-6); length=na)) :
                cand_grid(res, na)

            for b in b_cands
                b < b_lo && continue
                max_s = max(res - b, 0.0)
                for s in cand_grid(max_s, na)
                    c = res - b - s
                    c <= 0.0 && continue
                    cv = continuation_value_v4(p, w_grid, z_grid, shock, f_profile,
                                               next_slice, t, z, ell, b, s,
                                               x_A_new, x_B_new,
                                               ix_A_ns, ix_B_ns,  # stay
                                               ix_A_ns, ix_B_ns,  # reloc (tokens portable)
                                               REGIME_E2_2L)
                    v  = utility_crra_v4(c, p.gamma) + p.beta * cv
                    if v > best_v
                        best_v,best_c,best_b,best_s,best_xA,best_xB = v,c,b,s,x_A_new,x_B_new
                    end
                end
            end
        end
    end

    feasible = isfinite(best_v) && best_v > NEG_INF / 2.0
    return best_v, best_c, best_b, best_s, best_xA, best_xB, feasible
end

# ─────────────────────────────────────────────────────────────────────────────
# VFI main loop
# ─────────────────────────────────────────────────────────────────────────────

num_periods_v4(p::ModelParams_v4) = p.terminal_age - p.age0 + 1

function initialize_result_v4(p::ModelParams_v4, gs::GridSpec_v4)
    T    = num_periods_v4(p) + 1
    dims = (T, gs.n_w, gs.n_z, 2, gs.n_x_prev, gs.n_x_prev)
    return SolverResult_v4(
        fill(NEG_INF, dims), zeros(dims), zeros(dims), zeros(dims),
        zeros(dims), zeros(dims), falses(dims), Dict{String,Any}(),
    )
end

function terminal_slice_v4!(result::SolverResult_v4, p::ModelParams_v4,
                              w_grid::Vector{Float64}, gs::GridSpec_v4, t_last::Int)
    for (iw, w) in enumerate(w_grid), iz in 1:gs.n_z, iell in 1:2,
        ixA in 1:gs.n_x_prev, ixB in 1:gs.n_x_prev
        result.value[t_last, iw, iz, iell, ixA, ixB]    = utility_crra_v4(w, p.gamma)
        result.c_policy[t_last, iw, iz, iell, ixA, ixB] = w
        result.feasible[t_last, iw, iz, iell, ixA, ixB] = w >= 0.0
    end
end

function solve_v4(;
    params::ModelParams_v4  = default_params_v4(),
    grid_spec::GridSpec_v4  = default_grids_v4(),
    cfg::SolveConfig_v4     = default_config_v4(),
    regime::Int             = REGIME_E2_2L,
)
    w_grid      = build_w_grid_v4(grid_spec)
    z_grid      = build_z_grid_v4(grid_spec)
    x_prev_grid = build_xprev_grid_v4(grid_spec)
    result      = initialize_result_v4(params, grid_spec)
    f_profile   = income_profile_v4(params)
    shock       = build_shock_block_v4(params, cfg)

    t_last = num_periods_v4(params) + 1
    terminal_slice_v4!(result, params, w_grid, grid_spec, t_last)

    for t in (t_last - 1):-1:1
        age = params.age0 + t - 1
        if mod(age, 5) == 0
            @printf("  VFI age %d / %d\n", age, params.terminal_age)
            flush(stdout)
        end
        next_slice = view(result.value, t + 1, :, :, :, :, :)
        for (iw, w) in enumerate(w_grid),
            (iz, z) in enumerate(z_grid),
            iell in 1:2,
            ix_A_prev in 1:grid_spec.n_x_prev,
            ix_B_prev in 1:grid_spec.n_x_prev
            if w <= params.rho
                result.value[t, iw, iz, iell, ix_A_prev, ix_B_prev]    = NEG_INF
                result.feasible[t, iw, iz, iell, ix_A_prev, ix_B_prev] = false
                continue
            end
            v, c, b, s, xA, xB, ok = solve_state_v4(
                params, w_grid, z_grid, x_prev_grid, cfg, shock, f_profile,
                next_slice, t, w, z, iell, ix_A_prev, ix_B_prev, regime,
            )
            result.value[t, iw, iz, iell, ix_A_prev, ix_B_prev]     = v
            result.c_policy[t, iw, iz, iell, ix_A_prev, ix_B_prev]  = c
            result.b_policy[t, iw, iz, iell, ix_A_prev, ix_B_prev]  = b
            result.s_policy[t, iw, iz, iell, ix_A_prev, ix_B_prev]  = s
            result.xA_policy[t, iw, iz, iell, ix_A_prev, ix_B_prev] = xA
            result.xB_policy[t, iw, iz, iell, ix_A_prev, ix_B_prev] = xB
            result.feasible[t, iw, iz, iell, ix_A_prev, ix_B_prev]  = ok
        end
    end

    result.metadata["created_at"]      = string(Dates.now())
    result.metadata["regime"]          = regime_name_v4(regime)
    result.metadata["state_definition"]= "(t, w, z, ell, x_A_prev, x_B_prev)"
    result.metadata["n_x_prev"]        = grid_spec.n_x_prev
    result.metadata["x_prev_max"]      = grid_spec.x_prev_max
    result.metadata["tau_buy"]         = params.tau_buy
    result.metadata["tau_token"]       = params.tau_token
    result.metadata["tau_sell"]        = params.tau_sell
    result.metadata["rho_AB"]          = params.rho_AB
    result.metadata["p_relocate_working"] = params.p_relocate_working

    if cfg.save_path !== nothing
        open(cfg.save_path, "w") do io; serialize(io, result); end
    end
    return result, w_grid, z_grid, x_prev_grid, params
end

# ─────────────────────────────────────────────────────────────────────────────
# Summary
# ─────────────────────────────────────────────────────────────────────────────

function summary_v4(result::SolverResult_v4,
                     w_grid, z_grid, x_prev_grid,
                     params::ModelParams_v4, regime::Int)
    s = Dict{String,Any}()
    s["regime"]          = regime_name_v4(regime)
    s["total_points"]    = length(result.feasible)
    s["feasible_points"] = count(result.feasible)
    s["has_nan_value"]   = any(isnan, result.value)
    s["has_inf_value"]   = any(x -> isinf(x) && x > 0, result.value)

    iw_mid = max(1, div(length(w_grid), 2))
    iz_mid = max(1, div(length(z_grid), 2))
    ix_0   = 1  # index of x_prev = 0.0 (fresh entry; comparable to v3 t=1)

    s["V_t1_midpoint_ellA_xprev00"] = result.value[1, iw_mid, iz_mid, LOC_A, ix_0, ix_0]
    s["V_t1_midpoint_ellB_xprev00"] = result.value[1, iw_mid, iz_mid, LOC_B, ix_0, ix_0]

    for (lbl, iell) in [("ellA", LOC_A), ("ellB", LOC_B)]
        v1  = result.value[1, :, :, iell, ix_0, ix_0]
        f1  = result.feasible[1, :, :, iell, ix_0, ix_0]
        xAp = result.xA_policy[1, :, :, iell, ix_0, ix_0]
        xBp = result.xB_policy[1, :, :, iell, ix_0, ix_0]
        fv  = filter(isfinite, v1[f1])
        s["V_t1_mean_feasible_$lbl"]  = isempty(fv) ? nothing : mean(fv)
        s["mean_xA_xprev00_t1_$lbl"] = isempty(fv) ? nothing : mean(xAp[f1])
        s["mean_xB_xprev00_t1_$lbl"] = isempty(fv) ? nothing : mean(xBp[f1])
        s["xA_gt0_count_t1_$lbl"]    = count(x -> x > 0.0, xAp[f1])
        s["xB_gt0_count_t1_$lbl"]    = count(x -> x > 0.0, xBp[f1])
    end

    s["params"] = Dict(
        "gamma"              => params.gamma,  "beta"      => params.beta,
        "rf"                 => params.rf,     "rho"       => params.rho,
        "m"                  => params.m,      "delta_own" => params.rho - params.m,
        "sigma_h"            => params.sigma_h, "sigma_div" => params.sigma_div,
        "sigma_iota"         => params.sigma_iota, "rho_AB"=> params.rho_AB,
        "p_relocate_working" => params.p_relocate_working,
        "p_relocate_retired" => params.p_relocate_retired,
        "tau_sell"           => params.tau_sell, "tau_buy" => params.tau_buy,
        "tau_token"          => params.tau_token, "ltv_max"=> params.ltv_max,
    )
    return s
end

function print_summary_v4(s::Dict)
    println("v4_solver_summary:")
    for k in sort(collect(keys(s)))
        k == "params" && continue
        println("  $k: $(s[k])")
    end
    println("  params:")
    for (k, v) in s["params"]
        @printf("    %-24s %s\n", k * ":", v)
    end
end

# ─────────────────────────────────────────────────────────────────────────────
# Smoke test — struct-init, shock-block, and tx_cost checks. VFI not run.
# Run: julia src/vfi_solver_v4.jl --smoke-test
# ─────────────────────────────────────────────────────────────────────────────

function smoke_test_v4()
    println("=== v4 solver smoke test (no full VFI) ===")

    p   = default_params_v4()
    gs  = default_grids_v4(small=true)
    cfg = default_config_v4(small=true)

    @printf("  6D state: N_W=%d, N_Z=%d, N_ell=2, N_X_PREV=%d × %d\n",
            gs.n_w, gs.n_z, gs.n_x_prev, gs.n_x_prev)
    @printf("  x_prev_max=%.2f, tau_buy=%.4f, tau_token=%.4f, tau_sell=%.4f\n",
            gs.x_prev_max, p.tau_buy, p.tau_token, p.tau_sell)

    # Sigma decomposition
    check_sigma = abs(sqrt(p.sigma_div^2 + p.sigma_iota^2) - p.sigma_h) < 1e-8
    println("  sigma decomp: sqrt($(p.sigma_div)^2 + $(round(p.sigma_iota;digits=4))^2) = " *
            "$(round(sqrt(p.sigma_div^2+p.sigma_iota^2);digits=6)) ≈ $(p.sigma_h): $check_sigma")
    @assert check_sigma "sigma decomposition failed"

    # x_prev grid
    xp = build_xprev_grid_v4(gs)
    @printf("  x_prev_grid: %s\n", string(xp))
    @assert length(xp) == gs.n_x_prev
    @assert xp[1] == 0.0 "first x_prev must be 0.0"

    # 6D array allocation
    result = initialize_result_v4(p, gs)
    T      = num_periods_v4(p) + 1
    dims   = size(result.value)
    expected = (T, gs.n_w, gs.n_z, 2, gs.n_x_prev, gs.n_x_prev)
    @assert dims == expected "6D shape mismatch: got $dims expected $expected"
    bytes_per = prod(dims) * 8
    @printf("  6D shape: %s (%.1f KB / %.1f MB for 7 arrays)\n",
            string(dims), bytes_per/1024, 7*bytes_per/1024/1024)

    # Terminal slice
    w_grid = build_w_grid_v4(gs)
    z_grid = build_z_grid_v4(gs)
    terminal_slice_v4!(result, p, w_grid, gs, T)
    @assert !any(isnan, result.value[T,:,:,:,:,:]) "NaN in terminal slice"
    @assert all(result.feasible[T,:,:,:,:,:]) "infeasible in terminal slice"
    println("  terminal slice: PASS")

    # Shock block
    shock = build_shock_block_v4(p, cfg)
    @assert length(shock.weights) == cfg.quadrature_nodes^7
    @assert abs(sum(shock.weights) - 1.0) < 1e-8 "weights don't sum to 1"
    @assert any(shock.ra .!= shock.rb) "R_A == R_B everywhere; check rho_AB < 1"
    @printf("  shock block: %d points, weight sum=%.8f: PASS\n",
            length(shock.weights), sum(shock.weights))

    # tx_cost: buying both locations from zero
    let xA_p=0.0, xB_p=0.0, xA_n=xp[2], xB_n=xp[2]
        dA = xA_n - xA_p; dB = xB_n - xB_p
        tx = p.tau_buy*(max(dA,0)+max(dB,0)) + p.tau_token*(max(-dA,0)+max(-dB,0))
        expected_tx = p.tau_buy * (dA + dB)
        @assert abs(tx - expected_tx) < 1e-12 "tx_cost buy check: got $tx expected $expected_tx"
        @printf("  tx_cost buy (Δ=%.2f each): %.5f ≈ tau_buy*%.2f=%.5f: PASS\n",
                dA, tx, dA+dB, expected_tx)
    end

    # tx_cost: selling (negative delta) — tau_token for E2_2L
    let xA_p=xp[2], xA_n=0.0
        dA = xA_n - xA_p
        tx = p.tau_token * max(-dA, 0.0)
        expected_tx = p.tau_token * (-dA)
        @assert abs(tx - expected_tx) < 1e-12 "tx_cost sell check failed"
        @printf("  tx_cost token sell (Δ=%.2f): %.5f ≈ tau_token*%.2f: PASS\n",
                dA, tx, -dA)
    end

    # tx_cost: hold (no change) → zero cost
    let x = xp[2]
        tx = p.tau_buy*max(0.0,0.0) + p.tau_token*max(0.0,0.0)
        @assert tx == 0.0 "hold should have zero tx cost"
        println("  tx_cost hold (Δ=0): 0.0: PASS")
    end

    # E1_2L reloc indices: forced sell → (ix_0, ix_0)
    ix_0 = 1; ix_1 = argmin(abs.(xp .- 1.0))
    @printf("  E1_2L ix_0=%d (x=%.1f), ix_1=%d (x=%.2f)\n",
            ix_0, xp[ix_0], ix_1, xp[ix_1])
    @assert xp[ix_0] == 0.0 "ix_0 must map to x=0"

    # Housing cost checks
    @assert housing_cost_v4(0.0,0.0,LOC_A,p,REGIME_E0) == p.rho
    @assert housing_cost_v4(1.0,0.0,LOC_A,p,REGIME_E1_2L) == p.m
    @assert abs(housing_cost_v4(0.5,0.0,LOC_A,p,REGIME_E2_2L) -
                (p.rho - 0.5*(p.rho-p.m))) < 1e-12
    println("  housing_cost checks: PASS")

    # p_relocate checks
    @assert p_relocate_v4(p, 1)  == p.p_relocate_working
    @assert p_relocate_v4(p, 42) == p.p_relocate_retired
    println("  p_relocate checks: PASS")

    println("=== smoke_test_v4: PASS ===")
    return true
end

# ─────────────────────────────────────────────────────────────────────────────
# Entry point
# ─────────────────────────────────────────────────────────────────────────────

function main_v4(args::Vector{String}=ARGS)
    if "--smoke-test" in args
        smoke_test_v4()
        return
    end

    regime    = regime_from_env_v4()
    params    = default_params_v4()
    grid_spec = default_grids_v4()
    cfg       = default_config_v4()
    println("v4 solver — regime=$(regime_name_v4(regime)) — 6D state (t,w,z,ell,xA_prev,xB_prev)")
    @printf("  grids       : N_W=%d, N_Z=%d, N_X_PREV=%d (x_prev_max=%.2f)\n",
            grid_spec.n_w, grid_spec.n_z, grid_spec.n_x_prev, grid_spec.x_prev_max)
    @printf("  quadrature  : %d nodes, %d total\n",
            cfg.quadrature_nodes, cfg.quadrature_nodes^7)
    @printf("  mobility    : p_work=%.3f, p_ret=%.3f\n",
            params.p_relocate_working, params.p_relocate_retired)
    @printf("  tx costs    : tau_buy=%.3f (Δ>0), tau_token=%.3f (Δ<0 E2), tau_sell=%.3f (E1+forced)\n",
            params.tau_buy, params.tau_token, params.tau_sell)
    @printf("  returns     : rho_AB=%.2f, sigma_iota=%.4f\n",
            params.rho_AB, params.sigma_iota)
    flush(stdout)

    result, w_grid, z_grid, x_prev_grid, params_out =
        solve_v4(; params=params, grid_spec=grid_spec, cfg=cfg, regime=regime)
    s = summary_v4(result, w_grid, z_grid, x_prev_grid, params_out, regime)
    print_summary_v4(s)

    if get(ENV, "SUMMARY_JSON_PATH", "") != ""
        open(ENV["SUMMARY_JSON_PATH"], "w") do io
            write(io, JSON3.write(s))
        end
    end
end

if get(ENV, "TOKEN_PAPER_IMPORT_ONLY", "0") != "1" && abspath(PROGRAM_FILE) == @__FILE__
    main_v4()
end
