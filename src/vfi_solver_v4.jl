...
#
# Key extension vs v3:
#   - x_A_prev and x_B_prev are explicit state variables tracking previous-period
#     token holdings.  Transaction costs are charged on positive deltas (purchases)
#     and on negative deltas (token sales) each period:
#
...
#
#   - E1_2L: binary (x_ell ∈ {0,1}, x_{ell'}=0).  On relocation the sell-side cost
#     (tau_sell on x_ell_prev * R_ell) is applied in the wealth transition (unchanged
#     from v3).  tau_buy is NOT separately applied at relocation; instead the household
#     pays delta-based tau_buy in the period following relocation when it re-establishes
#     x_ell_new = 1 at the new location.  This is the correct, non-approximate treatment.
#
#   - E2_2L: continuous (x_A, x_B) ≥ 0.  Tokens are portable so no forced sale.
#     Pre-holding x_B at ell=A gives literal savings when/if the household later
#     increases x_B (expected tax: p_relocate * tau_buy per unit held pre-relocated).
#
#   - Interpolation for 6D value function: bilinear in (w, z) for each
#     (ell, ix_A_prev, ix_B_prev) index.
#
# Smoke test: julia src/vfi_solver_v4.jl --smoke-test
# VFI run:    REGIME=E1_2L julia src/vfi_solver_v4.jl
...
struct ModelParams_v4
    ...
    tau_sell::Float64
    tau_buy::Float64    # buying cost charged on positive deltas (per unit purchased)
    tau_token::Float64  # token-transfer cost charged on negative deltas (per unit sold)
    ltv_max::Float64
...
struct GridSpec_v4
    ...
    z_min::Float64
    z_max::Float64
    # x_prev grid: coarse by design (N_X_PREV points, uniform in [0, X_PREV_MAX])
    n_x_prev::Int
    x_prev_max::Float64
end
...
# 7D shock block: same dimensions as v3
...
# ─────────────────────────────────────────────────────────────────────────────
# Shock block — identical 7D GH quadrature to v3
# ─────────────────────────────────────────────────────────────────────────────
                        ...
                        for (i6, nu) in enumerate(nodes)
                            ...
                            for (i7, ne) in enumerate(nodes)
                                ...
                                wts[idx] = (weights[i1] * weights[i2] * weights[i3] *
                                            weights[i4] * weights[i5] *
                                            weights[i6] * weights[i7])
                            end
                        end
                    end
                end
...
# Net period housing cost.
# E1_2L: kappa rule identical to v3 (only occupied-location token saves rent).
# E2_2L: only the token at the *currently occupied* location saves rent;
#        x_{ell'} earns capital gain only (no same-period rent reduction).
...
# Per-period transaction cost on (x_A_prev, x_B_prev) -> (x_A_new, x_B_new).
# Positive delta: tau_buy charged.  Negative delta: tau_token charged.
...
# Wealth transition.
# E1_2L relocation: sell factor on the current-location token.
# E2_2L: tokens portable — no forced sale factor.
...
# Interpolation helper: look up V(w, z) for a given (ell, ix_A_prev, ix_B_prev) slice
# from the 6D next-period value array.
# next_value_6d: view of result.value[t+1, :, :, :, :, :]  → (n_w, n_z, 2, n_xp, n_xp)
function lookup_v_v4(next_value_6d::AbstractArray{Float64,5},
                     grids::Grids_v4,
                     w::Float64, z::Float64,
                     ell::Int, ix_A_prev::Int, ix_B_prev::Int)
    slice = view(next_value_6d, :, :, ell, ix_A_prev, ix_B_prev)
    return interp_bilinear_v4(slice, grids.w, grids.z, w, z)
end

# Nearest-index lookup for x_prev grid (flat grid, uniform spacing assumed).
function nearest_xprev_idx(xp_grid::Vector{Float64}, val::Float64)::Int
    n = length(xp_grid)
    n == 1 && return 1
    # clamp to grid
    val_c = clamp(val, xp_grid[1], xp_grid[end])
    # find nearest
    best_i = 1
    best_d = abs(xp_grid[1] - val_c)
    for i in 2:n
        d = abs(xp_grid[i] - val_c)
        if d < best_d
            best_d = d
            best_i = i
        end
...
# next_value_6d: (n_w, n_z, 2, n_xp, n_xp) view of value[t+1, ...]
# ─────────────────────────────────────────────────────────────────────────────

function continuation_value_v4(
    p::ModelParams_v4, grids::Grids_v4, shock::ShockBlock_v4,
    f_profile::Vector{Float64},
    next_value_6d::AbstractArray{Float64,5},
    ...
    # Sell factors for the relocation event.
    sf_A_stay  = 1.0; sf_B_stay  = 1.0
    sf_A_reloc = 1.0; sf_B_reloc = 1.0
    if regime == REGIME_E1_2L
        # On relocation: sell current-location unit at (1 - tau_sell).
        if ell == LOC_A
            sf_A_reloc = 1.0 - p.tau_sell
        else
            sf_B_reloc = 1.0 - p.tau_sell
        end
        # Note: the tau_buy cost for re-purchasing at the new location is
        # charged in the *next* period as a positive delta on x_{ell'_new}.
        # This is the correct Option 1 treatment (no explicit deduction here).
    end
    # E2_2L: tokens portable — sell factors remain 1.0.

    # After relocation, the x_prev state the household enters next period with:
    # - Stay:   (x_A_new, x_B_new) as chosen this period.
    # - Reloc:  If E1_2L: the occupied-location token is sold (position becomes 0);
    #           the household arrives next period with x_{ell'_new}=0 at new ell.
    #           In terms of (x_A_prev, x_B_prev) passed to the next-period state:
    #             relocating from A: x_A_prev_next = 0 (sold), x_B_prev_next = x_B_new=0 (admissibility)
    #             relocating from B: x_B_prev_next = 0, x_A_prev_next = 0
    #           For E2_2L: (x_A_prev_next, x_B_prev_next) = (x_A_new, x_B_new) regardless.

    # x_prev indices for the next period (nearest grid point)
    ix_A_stay  = nearest_xprev_idx(grids.x_prev, x_A_new)
    ix_B_stay  = nearest_xprev_idx(grids.x_prev, x_B_new)

    ix_A_reloc = ix_A_stay
    ix_B_reloc = ix_B_stay
    if regime == REGIME_E1_2L
        # After forced sale at relocation: both positions are 0 at the new location.
        ix_A_reloc = 1   # index of 0.0 (first point on [0, x_prev_max] grid)
        ix_B_reloc = 1
    end
    ...
    @inbounds for q in eachindex(shock.weights)
        ...
        v_stay  = lookup_v_v4(next_value_6d, grids, w_stay,  z_next, ell,     ix_A_stay,  ix_B_stay)
        v_reloc = lookup_v_v4(next_value_6d, grids, w_reloc, z_next, ell_alt, ix_A_reloc, ix_B_reloc)
...
function solve_state_v4(
    ...
    if regime == REGIME_E0
        # No housing: tx_cost = 0 (x_A_new = x_B_new = 0 always).
        # Note: x_A_prev, x_B_prev should be 0 at E0 too; if not, selling cost applies.
        sell_cost = p.tau_token * (x_A_prev + x_B_prev)
    ...
    elseif regime == REGIME_E1_2L
        ...
        # If previously owned (x_A_prev or x_B_prev > 0), pay tau_token for token sale.
        x_ell_prev = ell == LOC_A ? x_A_prev : x_B_prev
        sell_cost_rent = p.tau_token * x_ell_prev  # selling x_ell if switching to rent
        resources_rent = w - p.rho - sell_cost_rent
        ...
        # tx_cost = tau_buy * max(1 - x_ell_prev, 0): only pay on the increment.
        buy_cost_own = p.tau_buy * max(1.0 - x_ell_prev, 0.0)
        xA_own = ell == LOC_A ? 1.0 : 0.0
        xB_own = ell == LOC_B ? 1.0 : 0.0
        if w > 1.0 + p.m + buy_cost_own
            own_res = w - p.m - 1.0 - buy_cost_own
            b_lo    = -p.ltv_max * 1.0
        ...
        # Grid: X_total at nx points, alpha at nx points.
        # tx_cost = tau_buy * (max(dA,0) + max(dB,0)) + tau_token * (max(-dA,0) + max(-dB,0))
        delta_own = p.rho - p.m
        net_cost  = 1.0 - delta_own   # ≈ 0.96

        # Upper bound on X_total ignoring tx_cost (conservative; loop filters infeasible)
...
function terminal_slice_v4!(result::SolverResult_v4, p::ModelParams_v4,
                             grids::Grids_v4, t_last::Int, spec::GridSpec_v4)
    n_xp = spec.n_x_prev
...
function solve_v4(;
    ...
    terminal_slice_v4!(result, params, grids, t_last, grid_spec)
...
function summary_v4(result::SolverResult_v4, grids::Grids_v4, spec::GridSpec_v4,
    ...
    # Report t=1, initial state (x_A_prev=0, x_B_prev=0) — index (1, 1) in x_prev grid
    ...
    for (lbl, iell) in [("ellA", LOC_A), ("ellB", LOC_B)]
        # Slice over x_prev=0 initial state for comparability with v3 baseline
        v1   = result.value[1, :, :, iell, 1, 1]
        f1   = result.feasible[1, :, :, iell, 1, 1]
        xAp  = result.xA_policy[1, :, :, iell, 1, 1]
        xBp  = result.xB_policy[1, :, :, iell, 1, 1]
        feas_mask = findall(f1)
        feas_v = [v1[idx] for idx in feas_mask if isfinite(v1[idx])]
        s["V_t1_mean_feasible_xprev0_$lbl"]   = isempty(feas_v) ? nothing : mean(feas_v)
        s["mean_xA_t1_feasible_xprev0_$lbl"]  = isempty(feas_mask) ? nothing : mean([xAp[idx] for idx in feas_mask])
        s["mean_xB_t1_feasible_xprev0_$lbl"]  = isempty(feas_mask) ? nothing : mean([xBp[idx] for idx in feas_mask])
        s["xA_gt0_count_t1_xprev0_$lbl"]      = count(x -> x > 0.0, [xAp[idx] for idx in feas_mask])
        s["xB_gt0_count_t1_xprev0_$lbl"]      = count(x -> x > 0.0, [xBp[idx] for idx in feas_mask])
    end
...
# ─────────────────────────────────────────────────────────────────────────────
# Smoke test — no VFI; struct and mechanics checks only.
...
function smoke_test_v4()
    ...
    @printf("  tau_buy=%.4f (delta-based, not at-relocation approximation)\n", params.tau_buy)
    @printf("  tau_token=%.4f\n", params.tau_token)

    # sigma decomposition
    check_sigma = abs(sqrt(params.sigma_div^2 + params.sigma_iota^2) - params.sigma_h) < 1e-8
    @printf("  sigma decomposition: sqrt(%.6f^2 + %.6f^2) = %.6f  (sigma_h=%.6f)  OK=%s\n",
    ...
    @assert size(result.value, 5) == spec.n_x_prev  "x_A_prev dimension mismatch"
    @assert size(result.value, 6) == spec.n_x_prev  "x_B_prev dimension mismatch"

    # Memory estimate
    bytes_value = prod(dims) * 8
    @printf("  value array memory: %.1f MB  (all 6 arrays: %.1f MB)\n",
            bytes_value / 1e6, 7.0 * bytes_value / 1e6)

    # Terminal slice
    terminal_slice_v4!(result, params, grids, T, spec)
    ...
    @printf("  shock block: %d points  (expected %d^7 = %d)  weight_sum=%.10f\n",
            length(shock.weights), cfg.quadrature_nodes, expected_q, sum(shock.weights))

    # tx_cost computation
    p = params
    tc1 = tx_cost_v4(p, 0.0, 0.0, 0.5, 0.3)  # buying A: +0.5, buying B: +0.3
    expected_tc1 = p.tau_buy * (0.5 + 0.3)
    @assert abs(tc1 - expected_tc1) < 1e-12 "tx_cost buy mismatch: got $tc1, expected $expected_tc1"

    tc2 = tx_cost_v4(p, 0.5, 0.3, 0.2, 0.1)  # selling A: -0.3, selling B: -0.2
    expected_tc2 = p.tau_token * (0.3 + 0.2)
    @assert abs(tc2 - expected_tc2) < 1e-12 "tx_cost sell mismatch: got $tc2, expected $expected_tc2"

    tc3 = tx_cost_v4(p, 0.5, 0.0, 1.0, 0.5)  # buying A: +0.5, buying B: +0.5
    expected_tc3 = p.tau_buy * (0.5 + 0.5)
    @assert abs(tc3 - expected_tc3) < 1e-12 "tx_cost mixed mismatch"
    println("  tx_cost_v4 spot-checks: PASS")

    # Housing cost rule (same as v3 fixed rule)
    @assert housing_cost_v4(0.0, 0.0, LOC_A, p, REGIME_E0)    == p.rho
    @assert housing_cost_v4(1.0, 0.0, LOC_A, p, REGIME_E1_2L) == p.m     # owner at A
    @assert housing_cost_v4(0.0, 1.0, LOC_A, p, REGIME_E1_2L) == p.rho   # owns B but lives at A → renter
    kappa_e2 = housing_cost_v4(0.5, 0.5, LOC_A, p, REGIME_E2_2L)
    @assert abs(kappa_e2 - (p.rho - 0.5 * (p.rho - p.m))) < 1e-12 "E2_2L kappa rule mismatch"
    println("  housing_cost_v4 spot-checks: PASS")

    # x_prev nearest-index
    xp_grid = grids.x_prev
    @assert nearest_xprev_idx(xp_grid, 0.0)              == 1           "nearest idx at 0 wrong"
    @assert nearest_xprev_idx(xp_grid, xp_grid[end])     == spec.n_x_prev "nearest idx at max wrong"
    @assert nearest_xprev_idx(xp_grid, xp_grid[end] + 1) == spec.n_x_prev "nearest idx overflow wrong"
    println("  nearest_xprev_idx: PASS")

    # State-transition x_prev index for E1_2L relocation
    # After relocation from A: x_A_prev_next and x_B_prev_next should be index 1 (=0)
    @assert nearest_xprev_idx(xp_grid, 0.0) == 1 "relocation-zero index not 1"

    # E2_2L: pre-holding x_B at ell=A has strictly positive expected hedge benefit.
    # Quantify: expected per-period tau_buy saving from holding 1 unit of x_B = p_relocate * tau_buy
    hedge_premium_pp = p.p_relocate_working * p.tau_buy
    @printf("  hedge premium per period per unit x_B: p_reloc * tau_buy = %.4f * %.4f = %.6f\n",
            p.p_relocate_working, p.tau_buy, hedge_premium_pp)
    @assert hedge_premium_pp > 0.0 "hedge premium must be positive"
