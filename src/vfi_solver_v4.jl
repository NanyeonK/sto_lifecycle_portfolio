...
#   E1_2L — (c, b, s, ix_ell_choice)   binary own at current ell; x_{ell'}=0
#   E2_2L — (c, b, s, ix_A_choice, ix_B_choice) ∈ x_prev_grid × x_prev_grid
#
# Transaction costs applied at CHOICE TIME (per period, not only at relocation):
#   E2_2L:  tx = tau_buy*(max(δA,0)+max(δB,0)) + tau_token*(max(-δA,0)+max(-δB,0))
#           where δA = x_A_new - x_A_prev, δB = x_B_new - x_B_prev
#   E1_2L:  tx = tau_buy * max(x_ell_new - x_ell_prev, 0)
#           (sell at relocation still via sell_factor in wealth transition)
#
# Hedge mechanism: at ell=A, E2_2L household pre-buys x_B incrementally,
# paying tau_buy on small deltas. At relocation to B, delta_B is small or zero,
# avoiding the lump-sum tau_buy that E1_2L pays in full.
# Expected per-unit hedge saving per period: p_reloc * tau_buy ≈ 0.15%.
#
# Grid: N_X_PREV (default 3), range [0, X_PREV_MAX (default 1.0)].
# Choices are restricted to the x_prev_grid → enables exact state indexing.
# For E1_2L, choices x∈{0,1} are snapped to nearest x_prev_grid point.
#
# See: handoff/tau_buy_option1_spec.md
# Builds on: src/vfi_solver_v3.jl
...
struct ModelParams_v4
    ...
    tau_sell::Float64
    tau_buy::Float64      # buying cost per unit of positive delta
    tau_token::Float64    # token sell cost per unit of negative delta (E2_2L only)
...
struct SolveConfig_v4
    ...
    n_x_prev::Int         # points in x_prev_grid (default 3)
    x_prev_max::Float64   # upper bound of x_prev_grid (default 1.0)
...
mutable struct SolverResult_v4
    # 6D arrays indexed (t, iw, iz, iell, ixAprev, ixBprev)
    value::Array{Float64,6}
    c_policy::Array{Float64,6}
    b_policy::Array{Float64,6}
    s_policy::Array{Float64,6}
    xA_policy::Array{Float64,6}   # chosen x_A (= x_prev_grid[ix_A_choice])
...
# v4 defaults: N_W=15, N_Z=5 (reduced to compensate for 9x x_prev factor)
...
function build_grids_v4(spec::GridSpec_v4, cfg::SolveConfig_v4)
    return Grids_v4(build_w_grid_v4(spec), build_z_grid_v4(spec),
                    build_x_prev_grid(cfg))
end

# Snap continuous x to nearest index in x_prev_grid (for E1_2L binary choices).
function snap_to_xprev(x::Float64, xg::Vector{Float64})::Int
    best_i = 1
    best_d = abs(x - xg[1])
...
# Housing cost rule (same logic as v3, but only occupied unit saves rent in E2_2L).
...
# Per-period transaction cost on position change.
# E2_2L: tau_buy on increases, tau_token on decreases.
# E1_2L: tau_buy on increases only (sell handled by sell_factor in wealth transition).
@inline function tx_cost_v4(x_A_new::Float64, x_B_new::Float64,
    ...
    if regime == REGIME_E0
        return 0.0
    elseif regime == REGIME_E1_2L
        delta_ell = max(x_A_new - x_A_prev, 0.0) + max(x_B_new - x_B_prev, 0.0)
        return p.tau_buy * delta_ell
    else   # E2_2L
...
# ─────────────────────────────────────────────────────────────────────────────
# Bilinear interpolation over (w, z) slice
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Continuation value
# Integrates over quadrature draws × relocation Bernoulli.
# ix_A_choice, ix_B_choice: next-period x_prev indices (chosen this period).
# next_value_slice: view of result.value[t+1, :, :, :, :, :] — (n_w, n_z, 2, nx, nx)
# ─────────────────────────────────────────────────────────────────────────────
...
function continuation_value_v4(
    ...
    ix_A_choice::Int, ix_B_choice::Int,
    regime::Int,
)
    p_reloc = p_relocate_v4(p, t)
    ell_alt = ell == LOC_A ? LOC_B : LOC_A

    # Sell factors at relocation (E1_2L only; E2_2L tokens portable)
    ...
    @inbounds for q in eachindex(shock.weights)
        ...
        # Next period's x_prev state is (ix_A_choice, ix_B_choice) — same for
        # stay and reloc (portfolio unchanged; E2_2L tokens portable across moves;
        # E1_2L sell is captured by sell_factor above, not a separate state change here).
        v_stay  = interp_bilinear_v4(
            view(next_value_slice, :, :, ell,     ix_A_choice, ix_B_choice),
            grids.w, grids.z, w_stay,  z_next)
        v_reloc = interp_bilinear_v4(
            view(next_value_slice, :, :, ell_alt, ix_A_choice, ix_B_choice),
...
function solve_state_v4(
    ...
    elseif regime == REGIME_E1_2L
        ...
        # Both choices snapped to x_prev_grid; ix_{ell'} always 1 (value 0.0).
        ix_zero = snap_to_xprev(0.0, xg)   # index of 0.0 in x_prev_grid
        ix_one  = snap_to_xprev(1.0, xg)   # index nearest to 1.0

        # Case 1: rent (x_ell = 0)
        ix_A_c = ell == LOC_A ? ix_zero : ix_zero
        ix_B_c = ell == LOC_B ? ix_zero : ix_zero
        x_A_c = xg[ix_A_c]; x_B_c = xg[ix_B_c]
        tx = tx_cost_v4(x_A_c, x_B_c, x_A_prev, x_B_prev, p, regime)
                ...
                for s in candidate_grid(max_s, na)
                    ...
                    v = utility_crra(c, p.gamma) +
                        p.beta * continuation_value_v4(p, grids, shock, f_profile,
                                                       next_value_slice, t, z, ell,
                                                       b, s, x_A_c, x_B_c,
                                                       ix_A_c, ix_B_c, regime)
                    if v > best_v
                        best_v, best_c, best_b, best_s = v, c, b, s
                        best_xA, best_xB = x_A_c, x_B_c
                        best_ixA, best_ixB = ix_A_c, ix_B_c
                    end
                end
            end
        end

        # Case 2: own (x_ell = 1) — only at current location
        x_A_own = ell == LOC_A ? xg[ix_one] : 0.0
        x_B_own = ell == LOC_B ? xg[ix_one] : 0.0
        ix_A_own = ell == LOC_A ? ix_one : ix_zero
        ix_B_own = ell == LOC_B ? ix_one : ix_zero
        tx_own   = tx_cost_v4(x_A_own, x_B_own, x_A_prev, x_B_prev, p, regime)
        x_ell_own = xg[ix_one]   # the owned amount (≈1)
        kappa_own = housing_cost_v4(x_A_own, x_B_own, ell, p, regime)
        if w > x_ell_own + kappa_own + tx_own
            own_res = w - kappa_own - x_ell_own - tx_own
            ...
            for b in b_cands
                ...
                for s in candidate_grid(max_s, na)
                    ...
                    if v > best_v
                        ...
                        best_ixA, best_ixB = ix_A_own, ix_B_own
                    end
                end
            end
        end
        ...
        # Choices: (ix_A_choice, ix_B_choice) ∈ x_prev_grid × x_prev_grid.
        # Restricting x_new to x_prev_grid enables exact indexing of next-period state.
        delta_own = p.rho - p.m
        for ix_A_c in 1:nxp, ix_B_c in 1:nxp
            x_A_c = xg[ix_A_c]; x_B_c = xg[ix_B_c]
            X_total = x_A_c + x_B_c
            kappa   = housing_cost_v4(x_A_c, x_B_c, ell, p, regime)
            tx      = tx_cost_v4(x_A_c, x_B_c, x_A_prev, x_B_prev, p, regime)
            res     = w - kappa - X_total - tx
            res <= 0.0 && continue

            x_ell = ell == LOC_A ? x_A_c : x_B_c
...
function solve_v4(;
    ...
    for t in (t_last - 1):-1:1
        ...
        for (iw, w) in enumerate(grids.w),
            ...
            v, c, b, s, xA, xB, ixA_c, ixB_c, ok = solve_state_v4(
...
# ─────────────────────────────────────────────────────────────────────────────
# Summary (reports at initial x_prev = 0 state — household enters at t=1 with no holdings)
# ─────────────────────────────────────────────────────────────────────────────
...
function summary_v4(result::SolverResult_v4, grids::Grids_v4,
    ...
    # At initial state: ix_A_prev = ix_B_prev = 1 (no prior holdings)
    ix0 = 1
...
# ─────────────────────────────────────────────────────────────────────────────
# Smoke test — struct, grid, tx_cost, shock, terminal-slice checks. No VFI.
...
function smoke_test_v4()
    ...
    @assert length(grids.x_prev) == cfg.n_x_prev  "x_prev grid size wrong"
    ...
    nxp    = length(grids.x_prev)
    @printf("  value array: %s  (T=%d, nw=%d, nz=%d, nell=2, nxp=%d, nxp=%d)\n",
            string(dims), T, spec.n_w, spec.n_z, nxp, nxp)
    ...
    mem_mb = prod(dims) * 6 * 8 / 1e6   # 6 Float64 arrays + feasible BitArray
    @printf("  estimated memory: %.1f MB (6 Float64 arrays)\n", mem_mb)
    ...
    @assert length(shock.weights) == exp_q          "shock block size wrong"
    ...
    # 6. tx_cost_v4 spot-checks
    p = params
    # E2_2L: buying x_A from 0→0.5 at tau_buy=0.025
    tc1 = tx_cost_v4(0.5, 0.0, 0.0, 0.0, p, REGIME_E2_2L)
    @assert abs(tc1 - p.tau_buy * 0.5) < 1e-12  "tx_cost buy check failed"
    # E2_2L: selling x_A from 0.5→0 at tau_token=0.005
    tc2 = tx_cost_v4(0.0, 0.0, 0.5, 0.0, p, REGIME_E2_2L)
    @assert abs(tc2 - p.tau_token * 0.5) < 1e-12  "tx_cost sell check failed"
    # E2_2L: no change
    tc3 = tx_cost_v4(0.5, 0.5, 0.5, 0.5, p, REGIME_E2_2L)
    @assert tc3 == 0.0  "tx_cost no-change check failed"
    # E1_2L: buy x_ell from 0→1
    tc4 = tx_cost_v4(1.0, 0.0, 0.0, 0.0, p, REGIME_E1_2L)
    @assert abs(tc4 - p.tau_buy * 1.0) < 1e-12  "tx_cost E1_2L buy check failed"
    # E1_2L: sell x_ell (no tau_token)
    tc5 = tx_cost_v4(0.0, 0.0, 1.0, 0.0, p, REGIME_E1_2L)
    @assert tc5 == 0.0  "tx_cost E1_2L sell (no tau_token) check failed"
    println("  tx_cost_v4 spot-checks: PASS")

    # 7. snap_to_xprev checks
    xg = grids.x_prev
    @assert snap_to_xprev(0.0, xg) == 1            "snap 0.0 wrong"
    @assert snap_to_xprev(xg[end], xg) == nxp      "snap x_prev_max wrong"
    @assert snap_to_xprev(0.99, xg) >= 1            "snap 0.99 out of range"
    println("  snap_to_xprev checks: PASS")
