#!/usr/bin/env julia
# vfi_solver_v4.jl — Path B Option 1: 6D state with x_prev tracking and proper tau_buy on deltas
# v3 pivot: "Tokens decouple location from housing exposure" (2026-05-01)
# v4 extension: 2026-05-28 — full state extension per handoff/tau_buy_option1_spec.md
#
# State:    (t, w, z, ell, x_A_prev, x_B_prev)   — 6D
# Controls: regime-dependent:
#   E0      — (c, b, s)                 rent-only
#   E1_2L   — (c, b, s, x_A_new, x_B_new)  binary own at current location; x_{ell'}=0
#   E2_2L   — (c, b, s, x_A_new, x_B_new)  continuous fractional tokens at A and/or B
#
# Transaction cost (per period, on portfolio changes):
#   delta_A = x_A_new - x_A_prev
#   delta_B = x_B_new - x_B_prev
#   tx_cost = tau_buy   * (max(delta_A, 0) + max(delta_B, 0))
#           + tau_token * (max(-delta_A, 0) + max(-delta_B, 0))
#
# Mechanism: E2_2L household at ell=A may pre-hold x_B_prev > 0. On relocation to B,
# tokens are portable (no forced sale); next period x_B_prev = x_B_new is already large,
# so the tau_buy cost to reach full B exposure is reduced by tau_buy * x_B_prev.
# E1_2L is binary + location-tied: on relocation forced sale (tau_sell), next period
# x_prev = (0,0) and must pay tau_buy on full unit purchase.
#
# x_prev grid: N_X_PREV points, uniform on [0, X_PREV_MAX] (default 3 / 1.0)
# x_new is chosen from this grid to allow exact next-period state lookup.
#
# Default grids: N_W=15, N_Z=5 (reduced vs v3 to offset ~9x state expansion).
# Net compute factor: 9 * (15*5)/(21*7) ≈ 4.6x per regime vs v3 baseline.
...
struct ModelParams_v4
    ...
    # transaction costs (now all active in v4)
    tau_sell::Float64    # selling cost on forced E1_2L relocation (~0.06, NAR)
    tau_buy::Float64     # buying cost on positive delta x (~0.025)
    tau_token::Float64   # token selling cost on negative delta x (~0.01)
    # mortgage
    ltv_max::Float64
    r_mort_premium::Float64
    # v4: x_prev grid params
    n_x_prev::Int        # grid points for x_A_prev and x_B_prev
    x_prev_max::Float64  # maximum x in the x_prev grid
end
...
function default_grids_v4(; small::Bool=true)
    # Default N_W=15, N_Z=5 (reduced vs v3 to offset 9x x_prev expansion)
    if small
        return GridSpec_v4(
...
# Period housing cost — fixed kappa rule (occupied unit only saves rent).
# E0:    rho (renter)
# E1_2L: rho if x_ell<1, m if x_ell=1 (binary, current location only)
...
# Per-period transaction cost based on portfolio deltas.
...
# ─────────────────────────────────────────────────────────────────────────────
# Bilinear interpolation over (w, z); exact lookup over (ell, ixA, ixB)
# ─────────────────────────────────────────────────────────────────────────────
...
#   shape: (n_w, n_z, 2, n_xp, n_xp)  indexing (iw, iz, iell, ixA_prev, ixB_prev)
#
# ixA_new_stay, ixB_new_stay: grid indices of x_A_new, x_B_new chosen this period.
#   These become x_prev for the STAY branch (no relocation).
# ixA_new_reloc, ixB_new_reloc: x_prev indices for the RELOCATE branch.
#   E2_2L: same as stay (tokens portable, carry over to new location).
#   E1_2L: (1, 1) — forced sale zeroes both holdings at new location.
# ─────────────────────────────────────────────────────────────────────────────
...
function continuation_value_v4(
    ...
    ixA_new_stay::Int, ixB_new_stay::Int,
    ixA_new_reloc::Int, ixB_new_reloc::Int,
...
#
# x_A_new and x_B_new are restricted to the x_prev grid so that next-period
# state lookup is exact (no interpolation in the x_prev dimensions).
# ─────────────────────────────────────────────────────────────────────────────
...
function solve_state_v4(
    ...
    x_A_prev::Float64, x_B_prev::Float64,
    ixA_prev::Int, ixB_prev::Int,  # grid indices of x_A_prev, x_B_prev
    regime::Int,
)
    best_v  = NEG_INF
    best_c = best_b = best_s = best_xA_new = best_xB_new = 0.0
    na      = cfg.asset_grid_size
    xp_grid = grids.x_prev
    n_xp    = length(xp_grid)
    # index 1 always corresponds to x_prev=0.0 (zero holding)
    ix_zero = 1

    if regime == REGIME_E0
        resources = w - p.rho
        resources <= 0.0 && return best_v, best_c, best_b, best_s, 0.0, 0.0, false
        # E0: no housing; x_new = 0; tx_cost = tau_token * x_prev (selling previous holdings)
        # At entry into E0 this period, any previous holdings are disposed.
        tx = tx_cost_v4(0.0, 0.0, x_A_prev, x_B_prev, p)
        res2 = resources - tx
        res2 <= 0.0 && return best_v, best_c, best_b, best_s, 0.0, 0.0, false
        # stay: ixA_new=ix_zero, ixB_new=ix_zero; reloc: same
        for b in candidate_grid_v4(res2, na)
            max_s = max(res2 - b, 0.0)
            for s in candidate_grid_v4(max_s, na)
                ...
                v = utility_crra(c, p.gamma) +
                    p.beta * continuation_value_v4(
                        ...
                        t, z, ell, b, s, 0.0, 0.0,
                        ix_zero, ix_zero, ix_zero, ix_zero,
                        1.0, 1.0, regime)
                if v > best_v
                    best_v, best_c, best_b, best_s = v, c, b, s
                    best_xA_new = best_xB_new = 0.0
                end
            end
        end
    ...
    elseif regime == REGIME_E1_2L
        ...
        # x_prev grid index for 1.0: the last point (x_prev_max should be >= 1.0)
        ix_one = findlast(xp -> xp <= 1.0 + 1e-8, xp_grid)
        ix_one === nothing && (ix_one = n_xp)   # fallback

        # ── Case 1: rent (x_ell=0, x_ell'=0) ────────────────────────────────
        tx_rent = tx_cost_v4(0.0, 0.0, x_A_prev, x_B_prev, p)
        res_rent = w - p.rho - tx_rent
        if res_rent > 0.0
            # On relocation: no housing to sell; x_prev stays zero
        ...
        # ── Case 2: own at current location (x_ell=1, x_ell'=0) ─────────────
        xA_own = ell == LOC_A ? 1.0 : 0.0
        xB_own = ell == LOC_B ? 1.0 : 0.0
        ixA_own = ell == LOC_A ? ix_one : ix_zero
        ixB_own = ell == LOC_B ? ix_one : ix_zero
        tx_own  = tx_cost_v4(xA_own, xB_own, x_A_prev, x_B_prev, p)
        # Budget: c + m + 1 + tx_own + b + s = w  →  c = (w - m - 1 - tx_own) - b - s
        own_base = w - p.m - 1.0 - tx_own
        if own_base > 0.0
            b_lo = -p.ltv_max * 1.0
            b_cands = if p.ltv_max > 0.0
                collect(range(b_lo, max(own_base, b_lo + 1e-6); length=na))
            else
                candidate_grid_v4(own_base, na)
            end
            # On relocation from current ell: sell occupied unit (tau_sell)
            sf_A_reloc_own = ell == LOC_A ? (1.0 - p.tau_sell) : 1.0
            sf_B_reloc_own = ell == LOC_B ? (1.0 - p.tau_sell) : 1.0
            # After relocation, x_prev at new location = (0, 0) — forced sale
            for b in b_cands
                b < b_lo && continue
                max_s = max(own_base - b, 0.0)
                for s in candidate_grid_v4(max_s, na)
                    c = own_base - b - s
                    c <= 0.0 && continue
                    v = utility_crra(c, p.gamma) +
                        p.beta * continuation_value_v4(
                            ...
                            ixA_own, ixB_own,
                            ix_zero, ix_zero,   # after relocation x_prev=(0,0)
                            sf_A_reloc_own, sf_B_reloc_own, regime)
                    if v > best_v
                        best_v, best_c, best_b, best_s = v, c, b, s
                        best_xA_new, best_xB_new = xA_own, xB_own
                    end
                end
            end
        end
        ...
        # Continuous x_A_new, x_B_new — restricted to x_prev grid for exact lookup.
        # Iterate over all (ixA_new, ixB_new) grid index pairs.
        delta_own = p.rho - p.m

        for ixA_new in 1:n_xp
            x_A_new = xp_grid[ixA_new]
            for ixB_new in 1:n_xp
                x_B_new = xp_grid[ixB_new]
                ...
                # E2_2L: tokens portable on relocation — x_prev carries over to new location
                # sell factors remain 1.0; x_prev_next = (ixA_new, ixB_new) for both branches
                for b in b_cands
                    b < b_lo && continue
                    max_s = max(res - b, 0.0)
                    for s in candidate_grid_v4(max_s, na)
                        ...
                        v = utility_crra(c, p.gamma) +
                            p.beta * continuation_value_v4(
                                ...
                                ixA_new, ixB_new,   # relocate (tokens portable)
                                1.0, 1.0, regime)   # no sell cost on tokens
                        if v > best_v
                            best_v, best_c, best_b, best_s = v, c, b, s
                            best_xA_new, best_xB_new = x_A_new, x_B_new
                        end
                    end
                end
            end
    ...
    return best_v, best_c, best_b, best_s, best_xA_new, best_xB_new, feasible
end
...
function terminal_slice_v4!(result::SolverResult_v4, p::ModelParams_v4,
    ...
    for (iw, w) in enumerate(grids.w),
        ...
        ixA in 1:n_xp,
        ixB in 1:n_xp
        # At terminal period, consume all wealth. x_prev state doesn't affect terminal utility.
...
function solve_v4(;
    ...
    for t in (t_last - 1):-1:1
        ...
        for (iw, w) in enumerate(grids.w),
            ...
            v, c, b, s, xA_new, xB_new, ok = solve_state_v4(
            ...
            result.xA_new_policy[t, iw, iz, iell, ixA_prev, ixB_prev] = xA_new
            result.xB_new_policy[t, iw, iz, iell, ixA_prev, ixB_prev] = xB_new
...
# ─────────────────────────────────────────────────────────────────────────────
# Summary — reports initial x_prev = (0,0) slice (t=1 entry condition)
# ─────────────────────────────────────────────────────────────────────────────
...
function summary_v4(result::SolverResult_v4, grids::Grids_v4,
    ...
    # Report at initial state: t=1, x_A_prev=0, x_B_prev=0 (ix_zero=1)
    ...
    for (lbl, iell) in [("ellA", LOC_A), ("ellB", LOC_B)]
        # Slice at t=1, x_prev=(0,0) — entry condition for fresh households
        v1   = view(result.value,         1, :, :, iell, ix0, ix0)
        f1   = view(result.feasible,      1, :, :, iell, ix0, ix0)
        xAnp = view(result.xA_new_policy, 1, :, :, iell, ix0, ix0)
        xBnp = view(result.xB_new_policy, 1, :, :, iell, ix0, ix0)
        ...
        s["mean_xA_new_t1_feasible_$lbl"]    = isempty(feas_v) ? nothing : mean(xAnp[f1])
        s["mean_xB_new_t1_feasible_$lbl"]    = isempty(feas_v) ? nothing : mean(xBnp[f1])
        s["xA_new_gt0_count_t1_$lbl"]        = count(x -> x > 0.0, xAnp[f1])
        s["xB_new_gt0_count_t1_$lbl"]        = count(x -> x > 0.0, xBnp[f1])
    end

    # Hedge diagnostic: at t=1, ell=A, x_prev=(0,0): what fraction chooses x_B_new > 0?
    f1A  = view(result.feasible,      1, :, :, LOC_A, ix0, ix0)
    xBnA = view(result.xB_new_policy, 1, :, :, LOC_A, ix0, ix0)
    n_feas_A = count(f1A)
    s["hedge_frac_xBnew_gt0_ellA_xprev00"] = n_feas_A > 0 ?
        count(x -> x > 0.0, xBnA[f1A]) / n_feas_A : 0.0
    s["mean_xB_new_ellA_xprev00"] = n_feas_A > 0 ? mean(xBnA[f1A]) : 0.0
...
# ─────────────────────────────────────────────────────────────────────────────
# Smoke test — struct-init, 6D allocation, tx_cost, state update; no VFI.
...
function smoke_test_v4()
    ...
    @printf("  N_W=15, N_Z=5 (reduced defaults; override via env)\n")
    @printf("  tau_buy     = %.4f,  tau_token = %.4f\n", params.tau_buy, params.tau_token)
    @printf("  tau_sell    = %.4f\n", params.tau_sell)
    @printf("  rho_AB      = %.2f\n", params.rho_AB)
    ...
    n_xp = length(grids.x_prev)
    @printf("  x_prev grid: n=%d, values=%s\n", n_xp, string(grids.x_prev))
    @assert n_xp == params.n_x_prev "x_prev grid length mismatch"
    ...
    @printf("  value array memory: %.1f MB  (7 arrays total: ~%.1f MB)\n", mem_mb, 7*mem_mb)

    # Terminal slice
    terminal_slice_v4!(result, params, grids, T)
    @assert all(result.feasible[T, :, :, :, :, :]) "terminal states should all be feasible"
    @assert !any(isnan, result.value[T, :, :, :, :, :]) "NaN in terminal slice"
    println("  terminal slice: PASS")

    # tx_cost computation checks
    # (1) zero prev, positive new: pay tau_buy
    tc1 = tx_cost_v4(0.5, 0.3, 0.0, 0.0, params)
    expected_tc1 = params.tau_buy * (0.5 + 0.3)
    @assert abs(tc1 - expected_tc1) < 1e-12 "tx_cost: buy from zero failed (got $tc1, exp $expected_tc1)"
    # (2) reduction: pay tau_token
    tc2 = tx_cost_v4(0.2, 0.1, 0.5, 0.3, params)
    expected_tc2 = params.tau_token * (0.3 + 0.2)
    @assert abs(tc2 - expected_tc2) < 1e-12 "tx_cost: sell reduction failed (got $tc2, exp $expected_tc2)"
    # (3) no change: zero cost
    tc3 = tx_cost_v4(0.5, 0.5, 0.5, 0.5, params)
    @assert abs(tc3) < 1e-12 "tx_cost: no change should be 0 (got $tc3)"
    # (4) mixed: increase A, decrease B
    tc4 = tx_cost_v4(0.8, 0.1, 0.5, 0.5, params)
    expected_tc4 = params.tau_buy * 0.3 + params.tau_token * 0.4
    @assert abs(tc4 - expected_tc4) < 1e-12 "tx_cost: mixed delta failed (got $tc4, exp $expected_tc4)"
    println("  tx_cost_v4 checks: PASS")

    # x_prev = x_new identity: no change → zero cost
    x_test = grids.x_prev[2]   # second grid point
    tc_id  = tx_cost_v4(x_test, x_test, x_test, x_test, params)
    @assert abs(tc_id) < 1e-12 "identity (x_prev=x_new) should give zero tx_cost"
    ...
    # kappa = rho - x_A * delta_own  (only x_ell_local=x_A counts at ell=A)
    @assert abs(kappa_e2 - (p.rho - 0.5 * (p.rho - p.m))) < 1e-12 "E2_2L kappa at ell=A wrong"
    ...
    @printf("  shock block: %d points (n=%d^7), weight_sum=%.8f\n",
            expected_q, cfg.quadrature_nodes, sum(shock.weights))
    println("  shock block: PASS")

    # Hedge mechanism check: verify E2_2L reloc branch uses (ixA_new, ixB_new) not (1,1)
    # This is a structural check — the code passes same indices for stay and reloc in E2_2L
    println("  E2_2L token portability: indices stay=reloc in solve_state_v4 (verified in code)")

    # E1_2L reloc branch: (ix_zero, ix_zero) — forced to zero
    println("  E1_2L forced reloc: next x_prev=(0,0) after relocation (verified in code)")
...
function main_v4(args::Vector{String}=ARGS)
    ...
    grids_preview = build_grids_v4(grid_spec, params)
    n_xp      = length(grids_preview.x_prev)
    @printf("  grids     : N_W=%d, N_Z=%d, N_X_PREV=%d, X_PREV_MAX=%.2f\n",
            grid_spec.n_w, grid_spec.n_z, n_xp, params.x_prev_max)
    @printf("  state dim : 6D (%d x %d x %d x 2 x %d x %d)\n",
