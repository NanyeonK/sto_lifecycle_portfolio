...
# Writes: output/diagnostics/p6_option1_e2.json
...
export LTV_MAX=0.0
export GAMMA=5.0
export SUMMARY_JSON_PATH="$OUT_DIR/p6_option1_e2.json"

echo "=== Option-1 E2_2L run starting $(date) ===" | tee "$OUT_DIR/p6_option1_e2_stdout.log"
julia "$REPO_ROOT/src/vfi_solver_v4.jl" 2>&1 | tee -a "$OUT_DIR/p6_option1_e2_stdout.log"
echo "=== Done $(date) ===" | tee -a "$OUT_DIR/p6_option1_e2_stdout.log"
