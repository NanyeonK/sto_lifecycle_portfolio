#!/usr/bin/env bash
# Run E2_2L baseline with v4 solver (Option 1: full x_prev state)
# Usage: bash scripts/run_option1_e2.sh
# Server1 expected wall time: ~2-3 h (single thread, 4.6x v3)
...
export RHO_AB=0.50
export LTV_MAX=0.0
export SUMMARY_JSON_PATH="$OUT/p6_option1_e2.json"

echo "=== v4 E2_2L baseline (Option 1) ==="
julia "$REPO_ROOT/src/vfi_solver_v4.jl" 2>&1 | tee "$OUT/p6_option1_e2_stdout.log"
