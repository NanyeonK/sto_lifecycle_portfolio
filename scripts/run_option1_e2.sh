#!/usr/bin/env bash
# Run E2_2L baseline at v4 settings (Option 1 full state extension).
# Execute on server1 single thread.  Expected wall: ~2.5 h at defaults.
# Output: output/diagnostics/p6_option1_e2.json + stdout log.

set -euo pipefail

OUTDIR="${OUTDIR:-output/diagnostics}"
mkdir -p "$OUTDIR"

echo "=== v4 E2_2L baseline ==="
date
