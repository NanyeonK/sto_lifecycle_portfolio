#!/usr/bin/env bash
# run_option1_e2.sh — E2_2L with v4 6D state + per-period tau_buy on Δx (Option 1)
...
export SUMMARY_JSON_PATH="$REPO/output/diagnostics/p6_option1_e2.json"

echo "=== v4 E2_2L Option 1 ==="
date
JULIA_NUM_THREADS=1 julia "$REPO/src/vfi_solver_v4.jl" 2>&1 | tee "$REPO/output/diagnostics/p6_option1_e2_stdout.log"
echo "Done: $(date)"
