#!/usr/bin/env bash
# Run E2_2L baseline under Option 1 (v4 solver, 6D state extension)
...
echo "=== Option 1 E2_2L run ===" | tee output/diagnostics/p6_option1_e2_stdout.log
echo "  N_W=$N_W N_Z=$N_Z N_X_PREV=$N_X_PREV"  | tee -a output/diagnostics/p6_option1_e2_stdout.log
date | tee -a output/diagnostics/p6_option1_e2_stdout.log

JULIA_NUM_THREADS=1 julia src/vfi_solver_v4.jl \
    2>&1 | tee -a output/diagnostics/p6_option1_e2_stdout.log

echo "=== DONE E2_2L ===" | tee -a output/diagnostics/p6_option1_e2_stdout.log
date | tee -a output/diagnostics/p6_option1_e2_stdout.log
