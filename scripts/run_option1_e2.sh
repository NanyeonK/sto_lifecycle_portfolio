...
# Expected wall time: ~2-3 hours on server1 (single thread; 9x state factor vs v3).
...
OUTFILE="$OUT_DIR/p6_option1_e2.json"
LOGFILE="$OUT_DIR/p6_option1_e2_stdout.log"

echo "[$(date)] Running E2_2L v4 baseline..." | tee "$LOGFILE"
...
echo "[$(date)] E2_2L v4 DONE — output: $OUTFILE" | tee -a "$LOGFILE"
