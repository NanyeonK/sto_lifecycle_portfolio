#!/usr/bin/env bash
# Run E2_2L baseline with v4 solver (Path B Option 1)
# Execute on server1 from repo root: bash scripts/run_option1_e2.sh
set -euo pipefail

OUT=output/diagnostics
mkdir -p "$OUT"

echo "[run_option1_e2] Starting E2_2L v4 baseline..."
REGIME=E2_2L \
...
echo "[run_option1_e2] Done. JSON: $OUT/p6_option1_e2.json"
