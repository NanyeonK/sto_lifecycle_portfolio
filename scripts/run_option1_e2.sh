#!/usr/bin/env bash
# E2_2L baseline at v4 settings (Option 1 state extension)
...
echo "[$(date)] v4 E2_2L baseline starting (Option 1)"
time JULIA_NUM_THREADS=1 julia src/vfi_solver_v4.jl 2>&1 | tee "$OUTDIR/p6_option1_e2_stdout.log"
echo "[$(date)] v4 E2_2L baseline done. JSON: $SUMMARY_JSON_PATH"
