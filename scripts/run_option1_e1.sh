#!/usr/bin/env bash
# Run E1_2L baseline with v4 solver (Option 1: 6D state, proper tau_buy).
# Uses reduced grids: N_W=15, N_Z=5, N_X_PREV=3 to compensate for 6D state.
...
SUMMARY_JSON="$REPO/output/diagnostics/p6_option1_e1.json"
LOG_FILE="$REPO/output/diagnostics/p6_option1_e1_stdout.log"

echo "=== v4 E1_2L Option 1 run: $(date) ===" | tee "$LOG_FILE"
