...
|---|---|---|---|
| 1 | Open new branch `auto/2026-05-26-option1-state-extension` | cloud agent | branch on origin | DONE |
| 2 | Create `src/vfi_solver_v4.jl`: 6D state `(t, w, z, ell, x_A_prev, x_B_prev)` + tx_cost on deltas | cloud agent | file pushed | **DONE 2026-05-26** |
| 3 | Coarse `x_prev` grid N_X_PREV=3 {0.0, 0.75, 1.5}; N_W=15, N_Z=5 env-var configurable | cloud agent | in vfi_solver_v4.jl | **DONE 2026-05-26** |
| 4 | Smoke test stub `smoke_test_v4()` — 6D alloc, tx_cost, nearest-idx, terminal slice | cloud agent | `--smoke-test` flag | **DONE 2026-05-26** |
| 5 | Run `--smoke-test` on server1 | user/me | `output/diagnostics/p6_option1_smoke.md` | PENDING |
| 6 | Run E1_2L_v4 + E2_2L_v4 baselines: `bash scripts/run_option1_e1.sh` and `run_option1_e2.sh` | user/me | `p6_option1_e*.json` | PENDING |
| 7 | Compute CEV decomposition + write up | user/me | `p6_option1_decomposition.md` | PENDING |
