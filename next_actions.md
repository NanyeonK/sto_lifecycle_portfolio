...
| 2 | Create `src/vfi_solver_v4.jl`: 6D state `(t, w, z, ell, x_A_prev, x_B_prev)` + tx_cost on deltas | cloud agent | **DONE** `src/vfi_solver_v4.jl` (~550 LOC) |
| 3 | Use coarse `x_prev` grid: `N_X_PREV=3`; reduce N_W=15, N_Z=5 to compensate | cloud agent | **DONE** env-var configurable, defaults set |
| 4 | Smoke test stub `smoke_test_v4()` | cloud agent | **DONE** callable via `--smoke-test` |
...
**Server1 run commands:**
```bash
# Smoke test first (fast, no VFI):
julia src/vfi_solver_v4.jl --smoke-test

# E1_2L baseline (~45 min):
bash scripts/run_option1_e1.sh

# E2_2L baseline (~2-3 h):
