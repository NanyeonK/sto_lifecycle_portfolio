...
|---|---|---|---|
| 1 | Open new branch `auto/2026-05-04-option1-state-extension` | cloud agent | branch on origin |
| 2 | Create `src/vfi_solver_v4.jl`: 6D state `(t, w, z, ell, x_A_prev, x_B_prev)` + tx_cost on deltas | **DONE 2026-05-04** | `src/vfi_solver_v4.jl` (~530 LOC) |
| 3 | Use coarse `x_prev` grid: `N_X_PREV=3` ({0, 0.5, 1.0}); N_W=15, N_Z=5 | **DONE 2026-05-04** | env-var configurable in v4 |
| 4 | Smoke test stub `smoke_test_v4()` — 8 checks, no VFI | **DONE 2026-05-04** | callable via `--smoke-test` |
