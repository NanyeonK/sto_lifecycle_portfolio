...
| **2** | **Create `src/vfi_solver_v4.jl`: 6D state + tx_cost on deltas** | cloud agent | **DONE 2026-06-08** |
| **3** | **Coarse x_prev grid N_X_PREV=3 {0,0.5,1.0}; N_W=15, N_Z=5** | cloud agent | **DONE (embedded in v4)** |
| **4** | **Smoke test `smoke_test_v4()` — 6D alloc, tx_cost, state update** | cloud agent | **DONE (--smoke-test flag)** |
| 5 | `julia src/vfi_solver_v4.jl --smoke-test` on server1 | user/me | `output/diagnostics/p6_option1_smoke.md` |
| 6a | `bash scripts/run_option1_e1.sh` on server1 | user/me | `p6_option1_e1.json` |
| 6b | `bash scripts/run_option1_e2.sh` on server1 | user/me | `p6_option1_e2.json` |
| 7 | Compute CEV(E2_2L_v4 vs E1_2L_v4) + channel decomp | user/me or cloud | `p6_option1_decomposition.md` |
