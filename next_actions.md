...
|---|---|---|---|
| 1 | Open new branch `auto/2026-06-10-option1-state-extension` | cloud agent | branch on origin | **DONE 2026-06-10** |
| 2 | Create `src/vfi_solver_v4.jl`: 6D state `(t, w, z, ell, x_A_prev, x_B_prev)` + tx_cost on deltas | cloud agent | file pushed | **DONE 2026-06-10** |
| 3 | Use coarse `x_prev` grid: `N_X_PREV=3` ({0, 0.75, 1.5}); N_W=15, N_Z=5 to compensate | cloud agent | env-var configurable | **DONE 2026-06-10** |
| 4 | Smoke test stub `smoke_test_v4()` checking 6D allocation, tx_cost, interp_4d, state update | cloud agent | callable via `--smoke-test` | **DONE 2026-06-10** |
| 5 | Run scripts `scripts/run_option1_e1.sh` + `run_option1_e2.sh` written | cloud agent | scripts pushed | **DONE 2026-06-10** |
| 6 | Smoke test on server1: `julia src/vfi_solver_v4.jl --smoke-test` | user/me | `output/diagnostics/p6_option1_smoke.md` |
| 7 | Run E1_2L_v4 + E2_2L_v4 baselines (USER) | user/me | `p6_option1_e*.json` |
| 8 | Compute decomposition + write up | user/me | `p6_option1_decomposition.md` |
