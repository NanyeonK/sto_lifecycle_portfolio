...
|---|---|---|---|
| 1 | Open new branch `auto/2026-06-25-option1-state-extension` | cloud agent | branch on origin — DONE 2026-06-25 |
| 2 | Create `src/vfi_solver_v4.jl`: 6D state `(t, w, z, ell, x_A_prev, x_B_prev)` + tx_cost on deltas | cloud agent | file pushed — DONE 2026-06-25 |
| 3 | Use coarse `x_prev` grid: `N_X_PREV=3` (e.g., {0, 0.5, 1.0}); reduce N_W=15, N_Z=5 to compensate | cloud agent | env-var configurable — DONE 2026-06-25 |
| 4 | Smoke test stub `smoke_test_v4()` checking 6D allocation, tx_cost computation, state update consistency | cloud agent | callable via `--smoke-test` — DONE 2026-06-25 |
