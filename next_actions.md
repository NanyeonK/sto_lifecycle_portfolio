...
|---|---|---|---|
| 1 | Open new branch `auto/2026-05-31-option1-state-extension` | cloud agent | branch on origin | DONE |
| 2 | Create `src/vfi_solver_v4.jl`: 6D state `(t, w, z, ell, x_A_prev, x_B_prev)` + tx_cost on deltas | cloud agent | file pushed | DONE 2026-05-31 |
| 3 | Use coarse `x_prev` grid: `N_X_PREV=3` (e.g., {0, 0.5, 1.0}); reduce N_W=15, N_Z=5 to compensate | cloud agent | env-var configurable | DONE 2026-05-31 |
| 4 | Smoke test stub `smoke_test_v4()` checking 6D allocation, tx_cost computation, state update consistency | cloud agent | callable via `--smoke-test` | DONE 2026-05-31 |
| 5 | Run smoke test on server1: `julia src/vfi_solver_v4.jl --smoke-test` (USER) | user/me | `output/diagnostics/p6_option1_smoke.md` | PENDING |
| 6 | Run E1_2L_v4 + E2_2L_v4 baselines (USER): `bash scripts/run_option1_e1.sh` then `run_option1_e2.sh` | user/me | `p6_option1_e*.json` | PENDING |
