...
| 2 | Create `src/vfi_solver_v4.jl`: 6D state `(t, w, z, ell, x_A_prev, x_B_prev)` + tx_cost on deltas | cloud agent | **DONE** `src/vfi_solver_v4.jl` (928 LOC) |
| 3 | Use coarse `x_prev` grid: `N_X_PREV=3` (e.g., {0, 0.5, 1.0}); reduce N_W=15, N_Z=5 to compensate | cloud agent | **DONE** env-var configurable (`N_X_PREV`, `X_PREV_MAX`, `N_W`, `N_Z`) |
| 4 | Smoke test stub `smoke_test_v4()` checking 6D allocation, tx_cost computation, state update consistency | cloud agent | **DONE** callable via `--smoke-test`; 5 tx_cost checks, kappa checks, shape checks |
