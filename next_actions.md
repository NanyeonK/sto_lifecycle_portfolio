...
|---|---|---|---|
| 1 | Open new branch `auto/2026-05-15-option1-state-extension` | cloud agent | **DONE 2026-05-15** |
| 2 | Create `src/vfi_solver_v4.jl`: 6D state `(t, w, z, ell, x_A_prev, x_B_prev)` + tx_cost on deltas | cloud agent | **DONE 2026-05-15** (895 LOC) |
| 3 | Use coarse `x_prev` grid: `N_X_PREV=3` ({0,0.5,1.0}); reduce N_W=15, N_Z=5 to compensate | cloud agent | **DONE** (env-var configurable) |
| 4 | Smoke test stub `smoke_test_v4()` checking 6D allocation, tx_cost, state update | cloud agent | **DONE** (`--smoke-test` flag) |
| 4b | Run scripts: `scripts/run_option1_smoke.sh`, `run_option1_e1.sh`, `run_option1_e2.sh` | cloud agent | **DONE 2026-05-15** |
| 5 | **[USER]** Smoke test on server1: `bash scripts/run_option1_smoke.sh` | user/me | `output/diagnostics/p6_option1_smoke.log` |
| 6 | **[USER]** Run E1_2L_v4 + E2_2L_v4 baselines: `bash scripts/run_option1_e1.sh && bash scripts/run_option1_e2.sh` | user/me | `p6_option1_e*.json` |
| 7 | **[USER]** Compute decomposition + write up | user/me | `p6_option1_decomposition.md` |
