...
|---|---|---|---|
| 1 | Open new branch `auto/2026-06-17-option1-state-extension` | cloud agent | branch on origin |
| 2 | Create `src/vfi_solver_v4.jl`: 6D state `(t, w, z, ell, x_A_prev, x_B_prev)` + tx_cost on deltas | cloud agent | **DONE 2026-06-17** — `src/vfi_solver_v4.jl` (~540 LOC) |
| 3 | Use coarse `x_prev` grid: `N_X_PREV=3`; reduce N_W=15, N_Z=5 to compensate | cloud agent | **DONE** — env-var configurable; scripts at `scripts/run_option1_e{1,2}.sh` |
| 4 | Smoke test stub `smoke_test_v4()` checking 6D allocation, tx_cost, 4D interp, terminal slice | cloud agent | **DONE** — callable via `--smoke-test` flag |
