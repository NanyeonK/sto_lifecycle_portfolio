...
| 2 | Create `src/vfi_solver_v4.jl`: 6D state `(t, w, z, ell, x_A_prev, x_B_prev)` + tx_cost on deltas | cloud agent | **DONE** — `src/vfi_solver_v4.jl` (~580 LOC) |
| 3 | Use coarse `x_prev` grid: `N_X_PREV=3` (e.g., {0, 0.5, 1.0}); reduce N_W=15, N_Z=5 to compensate | cloud agent | **DONE** — env-var configurable; run scripts in `scripts/` |
| 4 | Smoke test stub `smoke_test_v4()` checking 6D allocation, tx_cost computation, state update consistency | cloud agent | **DONE** — `--smoke-test` flag; 4D interp check; one-period VFI spot-test |
| 5 | Smoke test on server1 (USER) | user/me | `bash scripts/run_option1_smoke.sh` → `output/diagnostics/p6_option1_smoke.log` |
| 6 | Run E1_2L_v4 + E2_2L_v4 baselines (USER) | user/me | `bash scripts/run_option1_e1.sh` + `run_option1_e2.sh` → `p6_option1_e*.json` |
