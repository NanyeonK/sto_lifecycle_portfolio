...
| 2 | Create `src/vfi_solver_v4.jl`: 6D state `(t, w, z, ell, ix_A_prev, ix_B_prev)` + tx_cost on deltas | cloud agent | **DONE** — `src/vfi_solver_v4.jl` (~560 LOC) |
| 3 | Coarse `x_prev` grid N_X_PREV=3, X_PREV_MAX=2.0 → {0.0,1.0,2.0}; N_W=15, N_Z=5 | cloud agent | **DONE** — env-var configurable |
| 4 | Smoke test stub `smoke_test_v4()` — 6D allocation, tx_cost, terminal slice, state-update | cloud agent | **DONE** — callable via `--smoke-test` |
| 5 | `julia src/vfi_solver_v4.jl --smoke-test` on server1 | user/me | `output/diagnostics/p6_option1_smoke.md` |
| 6 | Run `bash scripts/run_option1_e1.sh` + `run_option1_e2.sh` on server1 | user/me | `p6_option1_e1.json`, `p6_option1_e2.json` |
