...
|---|---|---|---|
| 1 | Open new branch `auto/2026-06-20-v4-solver-option1` | cloud agent | branch on origin |
| 2 | Create `src/vfi_solver_v4.jl`: 6D state `(t, w, z, ell, x_A_prev, x_B_prev)` + tx_cost on deltas | cloud agent | **DONE 2026-06-20** — `src/vfi_solver_v4.jl` (~580 LOC) |
| 3 | Use coarse `x_prev` grid: `N_X_PREV=3` (e.g., {0, 0.5, 1.0}); reduce N_W=15, N_Z=5 to compensate | cloud agent | **DONE** — env-var configurable; defaults in `default_grids_v4()` / `default_config_v4()` |
| 4 | Smoke test stub `smoke_test_v4()` checking 6D allocation, tx_cost computation, state update consistency | cloud agent | **DONE** — `--smoke-test` flag in `main_v4()` |
