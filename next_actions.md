...
|---|---|---|---|
| **DONE** | Open new branch `auto/2026-06-24-option1-state-extension` | cloud agent | branch on origin |
| **DONE** | Create `src/vfi_solver_v4.jl`: 6D state `(t, w, z, ell, x_A_prev, x_B_prev)` + tx_cost on deltas | cloud agent | `src/vfi_solver_v4.jl` (530 LOC) |
| **DONE** | Use coarse `x_prev` grid: `N_X_PREV=3` ({0, 0.75, 1.5}); reduce N_W=15, N_Z=5 to compensate | cloud agent | env-var configurable |
| **DONE** | Smoke test stub `smoke_test_v4()` checking 6D allocation, tx_cost computation, state update consistency | cloud agent | callable via `--smoke-test` |
| 5 | Run `julia src/vfi_solver_v4.jl --smoke-test` on server1 | user/me | stdout pass/fail |
| 6 | Run E1_2L_v4 + E2_2L_v4 baselines (USER) via `scripts/run_option1_e1.sh` / `_e2.sh` | user/me | `p6_option1_e*.json` |
