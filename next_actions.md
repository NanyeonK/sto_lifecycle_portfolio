...
|---|---|---|---|
| 1 | Open new branch `auto/2026-06-08-option1-state-extension` | cloud agent | **DONE** 2026-06-08 |
| 2 | Create `src/vfi_solver_v4.jl`: 6D state `(t, w, z, ell, x_A_prev, x_B_prev)` + tx_cost on deltas | cloud agent | **DONE** 2026-06-08 |
| 3 | Use coarse `x_prev` grid: `N_X_PREV=3` ({0, 0.75, 1.5}); N_W=15, N_Z=5 | cloud agent | **DONE** env-var configurable |
| 4 | Smoke test stub `smoke_test_v4()` — 6D alloc, tx_cost, interp_4d, housing_cost | cloud agent | **DONE** callable via `--smoke-test` |
| 5 | **Run smoke test on server1**: `julia src/vfi_solver_v4.jl --smoke-test` | **USER** | `output/diagnostics/p6_option1_smoke.md` |
| 6 | **Run baselines on server1**: `bash scripts/run_option1_e1.sh` then `run_option1_e2.sh` | **USER** | `p6_option1_e*.json` |
| 7 | Compute CEV + check H1/H2/H3 + write decomposition | user/me | `p6_option1_decomposition.md` |
