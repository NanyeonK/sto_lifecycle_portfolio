...
|---|---|---|---|
| 1 | DONE | Open new branch `auto/2026-05-30-option1-state-extension` | cloud agent | branch on origin |
| 2 | DONE | Create `src/vfi_solver_v4.jl`: 6D state `(t, w, z, ell, x_A_prev, x_B_prev)` + tx_cost on deltas | cloud agent | file committed |
| 3 | DONE | Coarse `x_prev` grid: `N_X_PREV=3` {0.0, 0.75, 1.5}; N_W=15, N_Z=5; all env-var configurable | cloud agent | in vfi_solver_v4.jl |
| 4 | DONE | Smoke test stub `smoke_test_v4()`: 6D alloc, tx_cost buy/sell/hold, terminal slice, shock block | cloud agent | `julia src/vfi_solver_v4.jl --smoke-test` |
| 5 | **NEXT (USER)** Smoke test on server1: `julia src/vfi_solver_v4.jl --smoke-test` | user/me | `output/diagnostics/p6_option1_smoke.md` |
| 6 | **NEXT (USER)** Run E1_2L_v4 + E2_2L_v4: `bash scripts/run_option1_e1.sh` and `run_option1_e2.sh` | user/me | `p6_option1_e1.json`, `p6_option1_e2.json` |
| 7 | Compute CEV(E2_2L_v4 vs E1_2L_v4); check H1 (mean_xB > 0), H2 (CEV > 4.255%), H3 (hedge channel) | user/me | `p6_option1_decomposition.md` |
