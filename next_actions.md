...
| 2 | **DONE 2026-06-26** Create `src/vfi_solver_v4.jl`: 6D state + tx_cost on deltas | cloud agent | `src/vfi_solver_v4.jl` pushed |
| 3 | **DONE 2026-06-26** `N_X_PREV=3`, N_W=15, N_Z=5 default grids; all env-var configurable | cloud agent | in vfi_solver_v4.jl |
| 4 | **DONE 2026-06-26** `smoke_test_v4()` — 6D alloc, tx_cost, kappa, p_relocate, shock block | cloud agent | callable via `--smoke-test` |
| 5 | Smoke test on server1 (USER): `julia src/vfi_solver_v4.jl --smoke-test` | user/me | `output/diagnostics/p6_option1_smoke.md` |
| 6 | Run E1_2L_v4 + E2_2L_v4 baselines: `bash scripts/run_option1_e1.sh` + `_e2.sh` | user/me | `p6_option1_e*.json` |
