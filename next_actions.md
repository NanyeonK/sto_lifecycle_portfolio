...
|---|---|---|---|
| 1 | Open new branch `auto/2026-05-02-option1-state-extension` | cloud agent | DONE 2026-05-20 — branch pushed |
| 2 | Create `src/vfi_solver_v4.jl`: 6D state `(t, w, z, ell, x_A_prev, x_B_prev)` + tx_cost on deltas | cloud agent | DONE 2026-05-20 — 950 LOC, smoke-test stub included |
| 3 | Use coarse `x_prev` grid: `N_X_PREV=3` (e.g., {0, 0.5, 1.0}); reduce N_W=15, N_Z=5 to compensate | cloud agent | DONE — env-var configurable; scripts use N_W=15, N_Z=5 |
| 4 | Smoke test stub `smoke_test_v4()` checking 6D allocation, tx_cost computation, state update consistency | cloud agent | DONE 2026-05-20 — 8 checks; run with `--smoke-test` |
