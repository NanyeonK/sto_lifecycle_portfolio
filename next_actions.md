...
|---|---|---|---|
| 1 | **DONE 2026-06-22** Open new branch `auto/2026-06-22-option1-state-extension` | cloud agent | branch on origin |
| 2 | **DONE 2026-06-22** Create `src/vfi_solver_v4.jl`: 6D state `(t, w, z, ell, x_A_prev, x_B_prev)` + tx_cost on deltas | cloud agent | `src/vfi_solver_v4.jl` (540 LOC) |
| 3 | **DONE 2026-06-22** Coarse `x_prev` grid: N_X_PREV=3 {0,0.5,1.0}; N_W=15, N_Z=5; all env-var configurable | cloud agent | embedded in v4 defaults |
| 4 | **DONE 2026-06-22** Smoke test stub `smoke_test_v4()`: 10 checks (6D alloc, tx_cost, terminal slice, shock block, kappa) | cloud agent | `julia src/vfi_solver_v4.jl --smoke-test` |
| 5 | Smoke test on server1: `bash scripts/run_option1_smoke.sh` | user/me | `output/diagnostics/p6_option1_smoke.md` |
| 6 | Run E1_2L_v4 + E2_2L_v4 baselines: `bash scripts/run_option1_e1.sh` then `run_option1_e2.sh` | user/me | `p6_option1_e1.json`, `p6_option1_e2.json` |
| 7 | Compute CEV decomposition + write up; check H1 (mean_xB>0), H2 (CEV>4.255%), H3 (hedge channel >0.5%) | user/me | `p6_option1_decomposition.md` |
